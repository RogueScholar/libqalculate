        ...
        if (v->isKnown()) {
          ...
          if (((KnownVariable *)v)->isExpression()) {
            ...
            if (!is_relative && ((KnownVariable *)v)->uncertainty().empty() &&
                ...
                ((KnownVariable *)v)
                        ->expression()
                        .find(CALCULATOR->f_interval->referenceName()) ==
                    string::npos) {
              fputs(" (", stdout);
      ...
      if ((!item->isHidden() || list_type == 'c') && item->isActive() &&
          (list_type != 'u' ||
           (item->subtype() != SUBTYPE_COMPOSITE_UNIT &&
            ((Unit *)item)->baseUnit() != CALCULATOR->u_euro)) &&
          (list_type != 'c' || ((Unit *)item)->isCurrency())) {
...
#define PRINT_AND_COLON_TABS(x, s)                                             \
  ...
  do {                                                                         \
    ...
    pctl += 8;                                                                 \
      ...
      str += b2oo(CALCULATOR->v_i->hasName("j") > 0, false);
      CHECK_IF_SCREEN_FILLED_PUTS(str.c_str())
...
#define STR_AND_TABS_7(s, sh, d, v, s0, s1, s2, s3, s4, s5, s6)                \
  ...
  CHECK_IF_SCREEN_FILLED_PUTS(str.c_str());
        ...
        STR_AND_TABS_BOOL(_("imaginary j"), "imgj",
                          _("Use 'j' (instead of 'i') as default symbol for "
                            "the imaginary unit."),
...
void setResult(Prefix *prefix, bool update_parse, bool goto_input,
  ...
  if (update_parse) {
    if (adaptive_interval_display) {
      if ((parsed_mstruct &&
           parsed_mstruct->containsFunction(CALCULATOR->f_uncertainty)) ||
        ...
        printops.interval_display = INTERVAL_DISPLAY_PLUSMINUS;
               ...
               parsed_mstruct->containsFunction(CALCULATOR->f_interval))
  ...
  if (do_stack) {
    ...
    if (do_mathoperation) {
      ...
      if (str2.length() == 1) {
        ...
        case '!': {
          CALCULATOR->calculateRPN(CALCULATOR->f_factorial, 0, evalops,
                                   parsed_mstruct);
          break;
        }
        case '>': {
        ...
        if (str2 == "**") {
          ...
          CALCULATOR->calculateRPN(CALCULATOR->f_factorial2, 0, evalops,
                                   parsed_mstruct);
          do_mathoperation = true;
