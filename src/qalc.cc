...
set_option_place:
    if(EQUALS_IGNORECASE_AND_LOCAL(svar, "base", _("base")) || EQUALS_IGNORECASE_AND_LOCAL(svar, "input base", _("input base")) || svar == "inbase" || EQUALS_IGNORECASE_AND_LOCAL(svar, "output base", _("output base")) || svar == "outbase") {
        ...
        else if(equalsIgnoreCase(svalue, "float32") || equalsIgnoreCase(svalue, "float")) {
            if(b_in) v = 0;
            else v = BASE_FLOAT32;
        }
        else if(equalsIgnoreCase(svalue, "float64") || equalsIgnoreCase(svalue, "double")) {
            if(b_in) v = 0;
            else v = BASE_FLOAT64;
        }
...
#define PUTS_CALENDAR(x, c) calstr = ""; BEGIN_BOLD(calstr); STR_AND_TABS((indentation ? string("  ") + x : x)); calstr += str; END_BOLD(calstr); b_fail = !dateToCalendar(date, y, m, d, c); if(b_fail) {calstr += _("failed");} else {calstr += i2s(d); calstr += " "; calstr += monthName(m, c, true); calstr += " "; calstr += i2s(y);} FPUTS_UNICODE(calstr.c_str(), stdout);
    ...
    if(!b_fail) {
        FPUTS_UNICODE((string(" (") + chineseStemName(st) + string(" ") + chineseBranchName(br) + ")").c_str(), stdout);
    }
    puts("");
...
void list_defs(bool in_interactive, char list_type = 0, string search_str = "") {
    ...
    if(!search_str.empty()) {
        ...
        if(name_list.empty()) {
            ...
            if(in_interactive) {
                CHECK_IF_SCREEN_FILLED_PUTS(_("For more information about a specific function, variable or unit, please use the info command (in interactive mode)."));
            }
            else {
                PUTS_UNICODE(_("For more information about a specific function, variable or unit, please use the info command (in interactive mode)."));
            }
            puts("");
        }
    } else if(list_type == 0) {
...
int main(int argc, char *argv[]) {
    ...
    for(int i = 1; i < argc; i++) {
        ...
        if(!calc_arg_begun && (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "-help") == 0 || strcmp(argv[i], "--help") == 0)) {
            ...
            fputs("\n", stdout);
            fputs("\t", stdout);
            PUTS_UNICODE(_("set the number base for results and, optionally, expressions"));
#ifdef HAVE_LIBCURL
            fputs("\n\t-e, -exrates\n", stdout);
            fputs("\t", stdout);
            ...
            fputs("]\n", stdout);
            fputs("\t", stdout);
            PUTS_UNICODE(_("displays a list of all user-defined or matching variables, functions and units"));
            ...
            fputs("\n", stdout);
            fputs("\t", stdout);
            PUTS_UNICODE(_("terminate calculation and display of result after specified amount of time"));
            fputs("\n\t-n, -nodefs\n", stdout);
            fputs("\t", stdout);
            PUTS_UNICODE(_("do not load any functions, units, or variables from file"));
            fputs("\n\t-nocurrencies\n", stdout);
            fputs("\t", stdout);
            ...
            fputs("]\n", stdout);
            fputs("\t", stdout);
            PUTS_UNICODE(_("start in programming mode (same as -b \"BASE BASE\" -s \"xor^\", with base conversion)"));
            fputs("\n\t-s, -set", stdout);
            fputs(" \"", stdout);
            ...
            fputs("\"\n", stdout);
            fputs("\t", stdout);
            PUTS_UNICODE(_("as set command in interactive program session (e.g. -set \"base 16\")"));
            fputs("\n\t-t, -terse\n", stdout);
            fputs("\t", stdout);
            PUTS_UNICODE(_("reduce output to just the result of the input expression"));
            fputs("\n\t-/+u8\n", stdout);
            fputs("\t", stdout);
...
#define PRINT_AND_COLON_TABS(x, s) str = x; pctl = unicode_length_check(x); if(strlen(s) > 0) {str += " (" s ")"; pctl += unicode_length_check(" (" s ")");} do {str += '\t'; pctl += 8;} while(pctl < 48);
            ...
            CHECK_IF_SCREEN_FILLED_PUTS(str.c_str())
            PRINT_AND_COLON_TABS(_("precision"), "prec") str += i2s(CALCULATOR->getPrecision());
            CHECK_IF_SCREEN_FILLED_PUTS(str.c_str())
            ...
            case BASE_CUSTOM: {
                str += CALCULATOR->customOutputBase().print(CALCULATOR->messagePrintOptions());
                break;
            }
            default: {
            ...
            str += b2oo(CALCULATOR->getVariableById(VARIABLE_ID_I)->hasName("j") > 0, false);
            CHECK_IF_SCREEN_FILLED_PUTS(str.c_str())
...
#define STR_AND_TABS_7(s, sh, d, v, s0, s1, s2, s3, s4, s5, s6) STR_AND_TABS_SET(s, sh); SET_DESCRIPTION(d); str += "(0"; if(v == 0) {str += "*";} str += " = "; str += s0; str += ", 1"; if(v == 1) {str += "*";} str += " = "; str += s1; str += ", 2"; if(v == 2) {str += "*";} str += " = "; str += s2; str += ", 3"; if(v == 3) {str += "*";} str += " = "; str += s3; str += ", 4"; if(v == 4) {str += "*";} str += " = "; str += s4; str += ", 5"; if(v == 5) {str += "*";} str += " = "; str += s5; str += ", 6"; if(v == 6) {str += "*";} str += " = "; str += s6; str += ")"; CHECK_IF_SCREEN_FILLED_PUTS(str.c_str());
                ...
                else if(printops.base > 2 && printops.base <= 36 && printops.base != BASE_OCTAL && printops.base != BASE_DECIMAL && printops.base != BASE_HEXADECIMAL) {
                    str += " ";
                    str += i2s(printops.base);
                    str += "*";
                }
                ...
                str += ", >= 0)";
                if(printops.min_exp != EXP_NONE && printops.min_exp != EXP_NONE && printops.min_exp != EXP_PRECISION && printops.min_exp != EXP_BASE_3 && printops.min_exp != EXP_PURE && printops.min_exp != EXP_SCIENTIFIC) {
                    str += " ";
                    str += i2s(printops.min_exp);
                    str += "*";
                }
                ...
                else if(evalops.parse_options.base > 2 && evalops.parse_options.base != BASE_OCTAL && evalops.parse_options.base != BASE_DECIMAL && evalops.parse_options.base != BASE_HEXADECIMAL) {
                    str += " ";
                    str += i2s(evalops.parse_options.base);
                    str += "*";
                }
                ...
                str += "(-1 = ";
                str += _("ask");
                if(auto_update_exchange_rates < 0) str += "*";
                str += ", 0 = ";
                str += _("never");
                if(auto_update_exchange_rates == 0) str += "*";
                str += ", > 0 = ";
    ...
    if(do_stack) {
        ...
        if(do_mathoperation) {
            ...
            if(str2.length() == 1) {
                ...
                case '-': {
                    CALCULATOR->calculateRPN(OPERATION_SUBTRACT, 0, evalops, parsed_mstruct);
                    break;
                }
                case '*': {
                    CALCULATOR->calculateRPN(OPERATION_MULTIPLY, 0, evalops, parsed_mstruct);
                    break;
                }
                case '/': {
                    CALCULATOR->calculateRPN(OPERATION_DIVIDE, 0, evalops, parsed_mstruct);
                    break;
                }
                case '&': {
                    CALCULATOR->calculateRPN(OPERATION_BITWISE_AND, 0, evalops, parsed_mstruct);
                    break;
                }
                case '|': {
                    CALCULATOR->calculateRPN(OPERATION_BITWISE_OR, 0, evalops, parsed_mstruct);
                    break;
                }
                case '~': {
                ...
                case '!': {
                    CALCULATOR->calculateRPN(CALCULATOR->getFunctionById(FUNCTION_ID_FACTORIAL), 0, evalops, parsed_mstruct);
                    break;
                }
                case '>': {
                    CALCULATOR->calculateRPN(OPERATION_GREATER, 0, evalops, parsed_mstruct);
                    break;
                }
                case '<': {
