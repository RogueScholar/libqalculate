...
struct tree_struct {
  ...
  void sort() {
    items.sort();
    for (list<tree_struct>::iterator it = items.begin(); it != items.end();
         ++it) {
      it->sort();
    }
  }
  bool operator<(tree_struct &s1) const { return item < s1.item; }
};
    ...
    if (CALCULATOR->units[i]->isActive() &&
      ...
      for (size_t i3 = 0; i3 < item->objects.size(); i3++) {
        ...
        if (CALCULATOR->units[i]->title() < u->title()) {
          b = true;
          item->objects.insert(item->objects.begin() + i3,
                               (void *)CALCULATOR->units[i]);
          break;
        }
      }
      if (!b)
        item->objects.push_back((void *)CALCULATOR->units[i]);
    }
  }
    ...
    if (CALCULATOR->variables[i]->isActive()) {
      ...
      for (size_t i3 = 0; i3 < item->objects.size(); i3++) {
        ...
        if (CALCULATOR->variables[i]->title() < v->title()) {
          b = true;
          item->objects.insert(item->objects.begin() + i3,
                               (void *)CALCULATOR->variables[i]);
          break;
        }
      }
      if (!b)
        item->objects.push_back((void *)CALCULATOR->variables[i]);
    }
  }
    ...
    if (CALCULATOR->functions[i]->isActive()) {
      ...
      for (size_t i3 = 0; i3 < item->objects.size(); i3++) {
        ...
        if (CALCULATOR->functions[i]->title() < f->title()) {
          b = true;
          item->objects.insert(item->objects.begin() + i3,
                               (void *)CALCULATOR->functions[i]);
          break;
        }
      }
      if (!b)
        item->objects.push_back((void *)CALCULATOR->functions[i]);
    }
  }
...
void print_function(MathFunction *f) {
  ...
  if (f->subtype() == SUBTYPE_DATA_SET) {
    ...
    fprintf(ffile,
            _("Retrieves data from the %s data set for a given object and "
              "property. If \"info\" is typed as property, all properties of "
              "the object will be listed."),
            f->title().c_str());
    fputs("</para>", ffile);
  }
  ...
  if (!f->example(true).empty()) {
    str = _("Example:");
    str += " ";
    str += fix(f->example(false, ename->name));
    fprintf(ffile, "<para>%s</para>\n", str.c_str());
  }
  if (f->subtype() == SUBTYPE_DATA_SET &&
  ...
  if (!f->condition().empty()) {
    ...
    fputs("<para>\n", ffile);
    fputs(fix(f->printCondition()).c_str(), ffile);
    fputs("\n", ffile);
    fputs("</para>\n", ffile);
    fputs("</formalpara>\n", ffile);
  }
...
void print_variable(Variable *v) {
  ...
  if (is_answer_variable(v)) {
    ...
    if (v == CALCULATOR->v_precision) {
      ...
      value = fix(
          CALCULATOR->localizeExpression(((KnownVariable *)v)->expression()));
      if (!((KnownVariable *)v)->uncertainty(&is_relative).empty()) {
        ...
        value += fix(CALCULATOR->localizeExpression(
      ...
      if (((KnownVariable *)v)
                  ->expression()
                  .find_first_not_of(NUMBER_ELEMENTS EXPS) == string::npos &&
          value.length() > 40) {
      ...
      if (!((KnownVariable *)v)->unit().empty() &&
        ...
        value += " ";
        value +=
            fix(CALCULATOR->localizeExpression(((KnownVariable *)v)->unit()));
      }
    } else {
    ...
    if (((UnknownVariable *)v)->assumptions()) {
      ...
      if (!value.empty() &&
          ((UnknownVariable *)v)->assumptions()->type() != ASSUMPTION_TYPE_NONE)
        value += " ";
  ...
  if (v->isApproximate() && !is_relative &&
      value.find(SIGN_PLUSMINUS) == string::npos) {
    if (v == CALCULATOR->v_pi || v == CALCULATOR->v_e ||
        v == CALCULATOR->v_euler || v == CALCULATOR->v_catalan) {
      value += " (";
...
void print_unit(Unit *u) {
  ...
  case SUBTYPE_ALIAS_UNIT: {
    ...
    if (au->firstBaseUnit()->subtype() == SUBTYPE_COMPOSITE_UNIT)
      base_unit = fix(((CompositeUnit *)au->firstBaseUnit())
                          ->print(false, true, printops.use_unicode_signs));
    else
      base_unit = au->firstBaseUnit()
    ...
    if (au->baseUnit() == CALCULATOR->u_euro && au->isBuiltin()) {
      ...
      if (!au->uncertainty(&is_relative).empty()) {
        if (is_relative) {
          relation += " (";
          relation += _("relative uncertainty");
          relation += ": ";
        } else
          relation += SIGN_PLUSMINUS;
        relation += fix(CALCULATOR->localizeExpression(au->uncertainty()));
        if (is_relative) {
          relation += ")";
        }
      }
      if (u->isApproximate() && !is_relative &&
          relation.find(SIGN_PLUSMINUS) == string::npos) {
        relation += " (";
  ...
  case SUBTYPE_COMPOSITE_UNIT: {
    base_unit = fix(
        ((CompositeUnit *)u)->print(false, true, printops.use_unicode_signs));
    relation = "";
    break;
  }
  }
...
int main(int, char *[]) {
  ...
  while (item) {
    ...
    if (item->objects.size() > 0) {
      fprintf(vfile,
              "<table id=\"qalculate-TBL-variables-%s\" frame=\"topbot\" "
              "colsep=\"1\">\n",
  ...
  while (item) {
    fprintf(ufile, "<sect%i id=\"qalculate-definitions-units-%i-%s\">\n", level,
            level, fixcat(item->item).c_str());
    fprintf(ufile, "<title>%s</title>\n", fix(item->item).c_str());
    if (item->objects.size() > 0) {
      fprintf(ufile,
              "<table id=\"qalculate-TBL-units-%s\" frame=\"topbot\" "
              "colsep=\"1\">\n",
              fixcat(item->item).c_str());
  ...
  if (!unit_cats.objects.empty()) {
    ...
    fprintf(
        ufile,
        "<table id=\"qalculate-TBL-units-%s\" frame=\"topbot\" colsep=\"1\">\n",
        _("Uncategorized"));
    fprintf(ufile, "<title>Units: %s</title>\n", _("Uncategorized"));
    fputs("<tgroup cols=\"4\" colsep=\"1\" rowsep=\"0\">\n", ufile);
    fputs("<colspec colname=\"COLSPEC0\"/>\n", ufile);
