...
int has_not_a_comparison() {
  if (!CALCULATOR->message())
    return 0;
  while (true) {
    if (CALCULATOR->message()->message() ==
        _("The calculation has been forcibly terminated. Please restart the "
          "application and report this as a bug."))
      return -1;
...
void test_integration5(const MathStructure &mstruct, const Number &a,
                       const Number &b) {
  cerr << "integrate(" << mstruct.print(CALCULATOR->messagePrintOptions())
       << ", " << a << "," << b << ")" << endl;
  ...
  if (i != 0)
    cout << i << "A: integrate("
         << mstruct.print(CALCULATOR->messagePrintOptions()) << ", " << a << ","
         << b << ")" << endl;
  struct timespec ts2;
  clock_gettime(CLOCK_MONOTONIC, &ts2);
  if (ts2.tv_sec > ts.tv_sec + 3)
    cout << "SLOW: integrate("
         << mstruct.print(CALCULATOR->messagePrintOptions()) << ", " << a << ","
         << b << ")" << endl;
  ...
  // if(!mstruct2.number().isReal()) {CALCULATOR->clearMessages(); imaginary++;
  // return;}
  if (!mstruct2.number().isReal())
    imaginary++;
  s1++;
  ...
  if (mstruct2.isNumber() && mstruct3.isNumber()) {
    if (!mstruct2.equals(mstruct3, true, true)) {
      ...
      if (nr1.isNonZero() || nr2.isNonZero()) {
        if (nr2.precision() >= 0)
          po.min_decimals = nr2.precision(true);
        else
          po.min_decimals = 0;
        str1 = nr1.print(po);
        str2 = nr2.print(po);
      ...
      if (nr_i1.isNonZero() || nr_i2.isNonZero()) {
        if (nr_i2.precision() >= 0)
          po.min_decimals = nr_i2.precision(true);
        else
          po.min_decimals = 0;
      ...
      if (!b_equal) {
        po.min_decimals = 0;
        cout << "Integration test: integrate("
             << mstruct.print(CALCULATOR->messagePrintOptions()) << ", " << a
             << "," << b << ")" << endl;
        display_errors();
        cout << str1 << endl;
...
void test_integration6(const MathStructure &mstruct, const Number &a,
                       const Number &b) {
  MathStructure x_var(CALCULATOR->v_x);
  cout << "Integration test: "
       << mstruct.print(CALCULATOR->messagePrintOptions()) << endl;
  MathStructure mstruct2(mstruct);
  EvaluationOptions eo;
  eo.parse_options.angle_unit = ANGLE_UNIT_RADIANS;
  eo.assume_denominators_nonzero = true;
  mstruct2.integrate(x_var, eo, true, -1, false, true, 4);
  if (mstruct2.containsFunction(CALCULATOR->f_integrate)) {
    clear_errors();
    return;
  }
...
void test_integration3(const MathStructure &mstruct,
                       const MathStructure &mstruct_arg) {
  MathStructure mstruct2(mstruct);
...
#define IS_INTERVAL(x) (x.isInterval(false))
  nr.setInterval(nr_minus_half, nr_half);
  nrs.push_back(nr);
  nr.setImaginaryPart(nr_one);
  nrs.push_back(nr);
  nr.setImaginaryPart(nr_minus_half);
  nrs.push_back(nr);
  nr.setImaginaryPart(nr_plus_inf);
  nrs.push_back(nr);
  nr.setInterval(nr_minus_one, nr_half);
  nrs.push_back(nr);
  nr.setInterval(nr_minus_half, nr_one);
  nrs.push_back(nr);
  nr.setInterval(nr_minus_one, nr_one);
  nrs.push_back(nr);
  nr.setInterval(Number(-2, 1), nr_two);
  nrs.push_back(nr);
  nr.setInterval(nr_zero, nr_half);
  nrs.push_back(nr);
  nr.setImaginaryPart(nrs[nrs.size() - 2]);
  nrs.push_back(nr);
  nr.setImaginaryPart(nr_one);
  nrs.push_back(nr);
  nr.setImaginaryPart(nr_minus_half);
  ...
  nr.setImaginaryPart(nr_plus_inf);
  nrs.push_back(nr);
  nr.setInterval(nr_minus_half, nr_zero);
  nrs.push_back(nr);
  nr.setInterval(nr_zero, nr_two);
  nrs.push_back(nr);
  nr.setInterval(nr_half, nr_one);
  nrs.push_back(nr);
  nr.setImaginaryPart(nrs[nrs.size() - 2]);
  nrs.push_back(nr);
  nr.setInterval(nr_one, nr_two);
  nrs.push_back(nr);
  nr.setInterval(nr_two, nr_three);
  nrs.push_back(nr);
  nr.setInterval(nr_minus_one, nr_minus_half);
  nrs.push_back(nr);
  nr.setImaginaryPart(nrs[nrs.size() - 2]);
  nrs.push_back(nr);
  nr.setInterval(nr_minus_one, Number(-2, 1));
  nrs.push_back(nr);
  nr.setInterval(Number(-2, 1), Number(-3, 1));
  nrs.push_back(nr);
  nr.setInterval(nr_minus_inf, nr_minus_one);
  nrs.push_back(nr);
  nr.setInterval(nr_minus_inf, nr_one);
  nrs.push_back(nr);
  nr.setInterval(nr_minus_inf, nr_plus_inf);
  nrs.push_back(nr);
  nr.setInterval(nr_plus_inf, nr_minus_one);
  nrs.push_back(nr);
  nr.setInterval(nr_plus_inf, nr_one);
  nrs.push_back(nr);
  nr.setImaginaryPart(nrs[nrs.size() - 2]);
  nrs.push_back(nr);
  nr.setImaginaryPart(nr_one);
  ...
  for (size_t i = 0; i < nrs.size(); i++) {
    for (size_t i2 = 0; i2 < nrs.size(); i2++) {
      ...
      nr.set(nrs[i]);
      if (nr.multiply(nrs[i2]))
        cout << nr.print(po) << endl;
      else
        cout << "FAILED" << endl;
      if (!INCLUDES_INFINITY(nr))
        nrsum3 += nr;
    }
    if (!INCLUDES_INFINITY(nrs[i])) {
      nrsum1 *= nrs[i];
      if (!IS_INTERVAL(nrs[i]))
        nrsum2 *= nrs[i];
    }
  }
...
string rnd_expression(int allow_unknowns, bool allow_functions,
                      int length_factor1 = 10, int length_factor2 = 5,
                      bool allow_units = false, bool allow_variables = false,
                      bool allow_interval = false, bool allow_complex = true,
                      bool only_integers = false, int num_ratio = 2,
                      bool num_ratio_den = false);

string rnd_unit() {
  int r = rand() % CALCULATOR->units.size();
  Unit *u = CALCULATOR->units[r];
  if (u->subtype() == SUBTYPE_COMPOSITE_UNIT)
    return ((CompositeUnit *)u)->print(false, true);
  string str;
  if (rand() % 3 == 0) {
...
string rnd_var() {
  ...
  while (true) {
    ...
    if (CALCULATOR->variables[r]->isKnown())
      return CALCULATOR->variables[r]->name();
  }
  return "";
}

string rnd_number(bool use_par = true, bool only_integers = false,
                  bool only_positive = false, bool allow_complex = true,
                  bool allow_interval = false) {
  string str;
  bool par = false;
  bool dot = only_integers;
  bool started = false;
  if (!only_positive && rand() % 4 == 0) {
    str += '-';
    par = true;
  }
  /*str += '0' + rand() % 10;
  ...
  while (true && str.length() < 20) {
    int r = rand();
    if (!started)
      r = r % (only_positive ? 9 + 1 : 10 + 1);
    else if (str.back() == '.')
      r = r % 10;
    else
      r = r % ((dot ? 19 : 20) + str.length() * 10);
    if (r > (dot ? 9 : 15))
      break;
    if ((r >= 10 && r <= 15) || (!dot && !started && r == 0)) {
      if (!started)
        str += '0';
      str += '.';
      dot = true;
  ...
  if (allow_complex && !only_integers && rand() % 10 == 0) {
    str += 'i';
    par = true;
  } else if (allow_interval && rand() % 2 == 0 &&
             str.find(".") == string::npos) {
    str += "+/-4E-8"; /*str += rnd_number(false, true, true, false, false);*/
  }
  if (par && use_par) {
    str += ')';
    str.insert(0, "(");
...
string rnd_item(int &par, bool allow_function = true, int allow_unknown = 1,
                int allow_unit = false, int allow_variable = false,
                bool allow_interval = false, bool allow_complex = true,
                bool only_integers = false, int num_ratio = 2,
                bool num_ratio_den = false) {
  int r = rand() % num_ratio + 1;
  string str;
  if ((num_ratio_den ? (r != 1) : (r == 1)) ||
      (!allow_unknown && !allow_function && !allow_unit && !allow_variable)) {
    str = rnd_number(true, only_integers, false, allow_complex, allow_interval);
  } else {
    if (allow_unit &&
        (rand() % (2 + allow_function + allow_unknown + allow_variable)) == 0) {
      str = rnd_unit();
    } else if (allow_variable &&
               (rand() % (2 + allow_function + allow_unknown)) == 0) {
      if (!allow_function && !allow_unknown && !allow_variable &&
          rand() % 2 == 0)
        str = rnd_number(true, only_integers, false, allow_complex,
                         allow_interval);
      else
        str = rnd_var();
    } else {
#define LAST_FUNC 23
      if (!allow_unknown) {
        if (allow_function)
          r = rand() % (LAST_FUNC - 4) + 4;
        else
          r = rand() % 2 + 4;
      } else {
        int au2 = 3 - allow_unknown % 3;
        r = (rand() %
             ((allow_function ? LAST_FUNC - 1 + allow_unknown : 5) - au2)) +
            4 - allow_unknown;
        if (r < 4 - allow_unknown % 3) {
          if (r < 0)
            r = -r;
          if (allow_unknown % 3 == 1)
            r = 3;
          else if (allow_unknown % 3 == 2)
            r = 2 + r % 2;
          else
            r = 1 + r % 3;
        }
      ...
      case 6: {
        str = "root(";
        str += rnd_expression(allow_unknown, allow_function, 6, 3, allow_unit,
                              allow_variable, allow_interval, allow_complex,
                              only_integers, num_ratio, num_ratio_den);
        str += ',';
      ...
      if (r > 5) {
        if (allow_unknown && rand() % 2 == 1) {
          ...
          str += rnd_item(par, true, allow_unknown, allow_unit, allow_variable,
                          allow_interval, allow_complex, only_integers,
                          num_ratio, num_ratio_den);
          par++;
        }
      }
    }
...
string rnd_operator(int &par, bool allow_pow = true) {
  ...
  case 5: {
    ...
    case 1:
      return str + "^";
    case 2:
      return str + "^2" + rnd_operator(par, false);
    case 3:
      return str + "^3" + rnd_operator(par, false);
    case 4:
      return str + "^-1" + rnd_operator(par, false);
    }
  }
  }
  return "";
...
string rnd_expression(int allow_unknowns, bool allow_functions,
                      int length_factor1, int length_factor2, bool allow_unit,
                      bool allow_variable, bool allow_interval,
                      bool allow_complex, bool only_integers, int num_ratio,
                      bool num_ratio_den) {
  int par = 0;
  string str;
  while (
      str.empty() ||
      rand() % ((length_factor1 - (int)str.length() / length_factor2 < 2)
                    ? 2
                    : (length_factor1 - (int)str.length() / length_factor2)) !=
          0) {
    str +=
        rnd_item(par, allow_functions, allow_unknowns, allow_unit,
                 allow_variable, allow_interval, allow_complex, only_integers);
    str += rnd_operator(par);
  }
  if (str.back() != ')')
    str += rnd_item(par, false, allow_unknowns, allow_unit, allow_variable,
                    allow_interval, allow_complex, only_integers);
  while (par > 0) {
    str += ")";
    par--;
...
int rt1 = 0, rt2 = 0, rt3 = 0, rt4 = 0, rt5 = 0, rt6 = 0, rt7 = 0, rt8 = 0,
    rt9 = 0;
void rnd_test(EvaluationOptions eo, int allow_unknowns, bool allow_functions,
              bool test_interval = true, bool test_equation = true,
              bool allow_unit = false, bool allow_variable = false,
              bool allow_interval = false) {
  bool b_iv = CALCULATOR->usesIntervalArithmetic();
  IntervalCalculation ic = eo.interval_calculation;
  cerr << "A0" << endl;
  string str = rnd_expression(allow_unknowns, allow_functions, 6, 4, allow_unit,
                              allow_variable, allow_interval);
  cerr << "A2:" << str << endl;
  PrintOptions po;
  po.interval_display = INTERVAL_DISPLAY_SIGNIFICANT_DIGITS;
  ...
  if(m3.isAborted()) {cout << str << " => " << mp << endl; cout << "ABORTED1b"
  << endl; CALCULATOR->useIntervalArithmetic(b_iv); return;} cerr << "A3c" <<
  endl; eo.interval_calculation = INTERVAL_CALCULATION_NONE;
  CALCULATOR->calculate(&m4, 5000, eo);
  if(m4.isAborted()) {cout << str << " => " << mp << endl; cout << "ABORTED1c"
  << endl; CALCULATOR->useIntervalArithmetic(b_iv); return;} cerr << "A3d" <<
  endl;*/
  ...
  if (m1.isNumber()) {
    ...
    /*if(m3.isNumber()) {
            if(COMPARISON_IS_NOT_EQUAL(m1.compare(m3))) {
                    rt2++;
                    cout << str << " => " << mp << endl;
                    cout << "UNEQUAL1b: " << m1.print(po) << ":" << m3.print(po)
    << endl;
            }
    }
    if(m4.isNumber()) {
            if(COMPARISON_IS_NOT_EQUAL(m1.compare(m4)) && m1.print(po) !=
    m4.print(po)) { rt2++; cout << str << " => " << mp << endl; cout <<
    "UNEQUAL1c: " << m1.print(po) << ":" << m4.print(po) << endl;
            }
    }*/
  }
  cerr << "A" << endl;
  ...
  if (mp.contains(CALCULATOR->v_x)) {
    ...
    if (nr.hasImaginaryPart() && rand() % 2 == 0)
      nr += Number(rnd_number(false, false, false, false, false));
    m1 = mp;
    ...
    if (m1.isAborted()) {
      cout << str << " => " << mp << endl;
      cout << "ABORTED4: " << nr << endl;
      CALCULATOR->useIntervalArithmetic(b_iv);
      return;
    }
    /*eo.interval_calculation = INTERVAL_CALCULATION_INTERVAL_ARITHMETIC;
    CALCULATOR->calculate(&m3, 5000, eo);
    if(m3.isAborted()) {cout << str << " => " << mp << endl; cout << "ABORTED4b:
    " << nr << endl; CALCULATOR->useIntervalArithmetic(b_iv); return;}
    eo.interval_calculation = ic;*/
    m2 = mp;
    ...
    /*if(COMPARISON_IS_NOT_EQUAL(m1.compare(m3))) {
            rt5++;
            cout << str << " => " << mp << ":" << nr << endl;
            cout << "UNEQUAL2a: " << m1.print(po) << ":" << m3.print(po) <<
    endl;
    }*/
    ...
    if (test_interval) {
      ...
      if (m1.isNumber() && m2.isNumber() && m1.number().precision(true) > 2) {
        ...
        if (m1 != m2) {
          ...
          if ((sr1 != sr2 && sr1.length() < 10) ||
              (si1 != si2 && si1.length() < 10)) {
            po.max_decimals = 1;
            if (sr1 != sr2)
              sr2 = m2.number().realPart().print(po);
            if (si1 != si2)
              si2 = m2.number().imaginaryPart().print(po);
            if ((sr1 != sr2 && sr1.length() < 10) ||
                (si1 != si2 && si1.length() < 10)) {
              rt9++;
              cout << str << " => " << mp << ":" << nr_iv << endl;
              cout << "UNEQUAL4: " << m1.print(po) << ":" << m2.print(po)
                   << endl;
            }
          }
        }
    ...
    if (test_equation) {
      ...
      if (m1.isComparison() && m1.comparisonType() == COMPARISON_EQUALS &&
          m1[0] == CALCULATOR->v_x) {
        m2 = mp;
        ...
        if (m2.isNumber()) {
          ...
          if (m2.number().isNonZero()) {
            ...
            if (nr > -10) {
              rt7++;
              cout << str << " => " << mp << endl;
              cout << "UNEQUAL3: " << m1.print(po) << ":" << m2.print(po)
                   << endl;
            }
          }
        }
        ...
        if (m1.type() == STRUCT_LOGICAL_AND) {
          for (size_t i = 0; i < m1.size(); i++) {
            if (m1[i].isComparison() &&
                m1[i].comparisonType() != COMPARISON_EQUALS) {
              eo.approximation = APPROXIMATION_APPROXIMATE;
              CALCULATOR->calculate(&m1[i], 5000, eo);
              if (m1[i].isAborted()) {
                cout << str << " => " << mp << endl;
                cout << "ABORTED9: " << m1[i] << endl;
                CALCULATOR->useIntervalArithmetic(b_iv);
                return;
              }
              if (m1[i].isZero()) {
            ...
            if (m1[i].isComparison() &&
              ...
              if (m1[i].type() == STRUCT_LOGICAL_AND) {
                for (size_t i2 = 0; i2 < m1[i].size(); i2++) {
                  if (m1[i][i2].isComparison() &&
                      m1[i][i2].comparisonType() != COMPARISON_EQUALS) {
                    eo.approximation = APPROXIMATION_APPROXIMATE;
                    CALCULATOR->calculate(&m1[i][i2], 5000, eo);
                    if (m1[i][i2].isAborted()) {
                      cout << str << " => " << mp << endl;
                      cout << "ABORTED9: " << m1[i][i2] << endl;
                      CALCULATOR->useIntervalArithmetic(b_iv);
                      return;
                    }
                    if (m1[i][i2].isZero()) {
                    ...
                    if (m2.isNumber()) {
                      ...
                      if (m2.number().isNonZero()) {
                        ...
                        if (nr > -10) {
                          rt7++;
                          cout << str << " => " << mp << endl;
                          cout << "UNEQUAL3: " << m1[i][i2].print(po) << ":"
                               << m2.print(po) << endl;
                        }
                      }
                    }
        ...
        if (b_iv != CALCULATOR->usesIntervalArithmetic()) {
          cout << "INTERVAL ARITHMETIC CHANGED5: " << str << " => " << mp
               << endl;
          CALCULATOR->useIntervalArithmetic(b_iv);
        }
      }
  ...
  string str2 = rnd_expression(allow_unknowns, allow_functions, 5, 4,
                               allow_unit, allow_variable, allow_interval);
  str.insert(0, "(");
  str += ") / (";
  str += str2;
  str += ")";
  ...
  CALCULATOR->calculate(&m3, 5000, eo);
  if(m3.isAborted()) {cout << str << " => " << mp << endl; cout << "ABORTED1b"
  << endl; CALCULATOR->useIntervalArithmetic(b_iv); return;}
...
void speed_test() {
  ...
  /*MathStructure m = CALCULATOR->parse("sin('y')+'y'+'y'+'x'^2/3+'x'+'y'",
  eo.parse_options); MathStructure mx("x"); MathStructure my("y"); for(size_t i
  = 0; i < 500000; i++) { MathStructure m2(m);
      //m2.replace(mx, m_one);
      //m2.replace(my, m_one);
  ...
  eo.expand = false;
  m.eval(eo);
  MathStructure v1 = m.generateVector(CALCULATOR->v_x, 1, 100, 800000, NULL,
  eo); m = CALCULATOR->parse("1/x"); m.eval(eo); MathStructure v2 =
  m.generateVector(CALCULATOR->v_x, 1, 100, 800000, NULL, eo); m =
  CALCULATOR->parse("x/4-3"); m.eval(eo); MathStructure v3 =
  m.generateVector(CALCULATOR->v_x, 1, 100, 800000, NULL, eo); m =
  CALCULATOR->parse("sin(x)", eo.parse_options); m.eval(eo);
  MathStructure v4 = m.generateVector(CALCULATOR->v_x, 1, 100, 800000, NULL,
  eo);*/
...
bool test_float(const Number &nr) {
  ...
  if (nr2.print(po) != sbin) {
    if (nr2.isZero() && sbin[0] == '1' && sbin.find("1", 1) == string::npos)
      return true;
...
int main(int argc, char *argv[]) {
  ...
  for(size_t i = 0; i < 100000000L; i++) {
      ...
      //if(po.min_decimals > 1 && po.max_decimals > po.min_decimals)
  po.max_decimals = po.min_decimals; po.min_exp = -(i % 3); if(po.min_exp == 0)
  po.min_exp = 1; b = a; b.setToFloatingPoint(); po.use_max_decimals = false;
      po.min_decimals = 100;
      ...
      str = a.print(po);
      if(str != str2) {
              cout << str << ":" << str2 << ":" << po.min_decimals << ":" <<
  po.max_decimals << endl; po.interval_display = INTERVAL_DISPLAY_INTERVAL;
              //po.min_decimals = 20;
              //po.max_decimals = -1;
  ...
  /*for(size_t i = 0; i < 50000;) {
      str = rnd_expression(10, true, 6, 4, false, false, false, false, false, 5,
  false); CALCULATOR->parse(&mp, str, evalops.parse_options); cerr << str <<
  endl; if(mp.contains(CALCULATOR->v_x)) { a.set(rnd_number(false, false, false,
  false, false)); b.set(rnd_number(false, false, false, false, false)); cerr <<
  "A" << endl; if(a < b) test_integration5(mp, a, b); else test_integration5(mp,
  b, a); i++; if(i % 1000 == 0) cout << f1 << ":" << s1 << ":" << s2 << ":" <<
  successes << ":" << imaginary << endl;
      }
  }
  cout << successes << ":" << imaginary << endl;
  return 0;*/
  ...
  for (size_t i = 0; i <= 15000; i++) {
    /*string str = rnd_expression(17, false, 20, 4, false, false, false, false,
    true); cout << str << endl; MathStructure mstruct;
    CALCULATOR->calculate(&mstruct, str, 10000, evalops);
    mstruct.format(po);
    cout << mstruct.print() << endl;
    if(mstruct.isAborted()) break;*/
    // if(mstruct.isPower() || (mstruct.isMultiplication() &&
    // !mstruct.containsType(STRUCT_DIVISION))) cout << str << "\n" << mstruct <<
    // endl;
    rnd_test(evalops, 4, true, false, false, false, false, false);
    if (i % 1000 == 0)
      cout << endl
           << rt1 << ":" << rt2 << ":" << rt3 << ":" << rt4 << ":" << rt5 << ":"
           << rt6 << ":" << rt7 << ":" << rt8 << ":" << rt9 << endl
           << endl;
  }
  cout << endl
       << endl
