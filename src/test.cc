...
void test_integration4(const MathStructure &mstruct) {
  ...
  cout << "Integration test: " <<
  mstruct.print(CALCULATOR->messagePrintOptions()) << endl; MathStructure
  mstruct2(mstruct); EvaluationOptions eo; eo.parse_options.angle_unit =
  ANGLE_UNIT_RADIANS; eo.assume_denominators_nonzero = true;
  mstruct2.integrate(x_var, eo, true, 1, false, true, 4);
  if(mstruct2.containsFunction(CALCULATOR->f_integrate)) {clear_errors();
  return;} mstruct2.differentiate(x_var, eo); mstruct2.eval(eo); MathStructure
  mstruct3(mstruct2); mstruct3.replace(x_var, 3); mstruct3.eval(eo);
  display_errors();
...
string rnd_expression(int allow_unknowns, bool allow_functions,
                      ...
                      bool allow_complex, bool only_integers) {
  int par = 0;
  string str;
  while (
      str.empty() ||
