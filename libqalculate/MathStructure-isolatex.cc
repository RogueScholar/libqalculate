      ...
      if (CHILD(0)[1].contains(x_var)) {
        if ((ct_comp == COMPARISON_EQUALS ||
          ...
          if (CHILD(0)[0].representsNonComplex() &&
            ...
            MathStructure *mreq3a = new MathStructure(*marg);
            ...
            MathStructure *mreq3b = new MathStructure(*marg);
  ...
  case STRUCT_FUNCTION: {
    if (CHILD(0).function()->id() == FUNCTION_ID_ROOT && VALID_ROOT(CHILD(0))) {
      ...
      if (b_zero && f->id() == FUNCTION_ID_COS) {
        switch (eo.parse_options.angle_unit) {
        case ANGLE_UNIT_DEGREES: {
          CHILD(1).add(Number(-90, 1), true);
          break;
        }
        case ANGLE_UNIT_GRADIANS: {
          CHILD(1).add(Number(-100, 1), true);
          break;
        }
        case ANGLE_UNIT_RADIANS: {
          CHILD(1).add(CALCULATOR->getVariableById(VARIABLE_ID_PI), true);
          CHILD(1).last() *= nr_minus_half;
          break;
        }
        default: {
          CHILD(1).add(CALCULATOR->getVariableById(VARIABLE_ID_PI), true);
          CHILD(1).last() *= nr_minus_half;
          CHILD(1).last() *= CALCULATOR->getRadUnit();
        }
        }
      }
      if (b_one) {
