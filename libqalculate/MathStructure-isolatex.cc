            ...
            if(CALCULATOR->usesIntervalArithmetic()) {
                ...
                if(mtest2.isNumber()) {
                    if(mtest.comparisonType() == COMPARISON_LESS || mtest.comparisonType() == COMPARISON_EQUALS_LESS) {
                        ...
                        else if(mtest2.number().isInterval() || mtest2[0].number().isRational()) {
                            mthis = m_one;
                            return 0;
                        }
                    } else {
                        if(!mtest2.number().isNonZero()) return 1;
                        else if(mtest2.number().isInterval() || mtest2.number().isRational()) {
                            mthis.clear();
                            return 0;
                        }
                    }
                    ...
                    if(mtest.comparisonType() == COMPARISON_LESS || mtest.comparisonType() == COMPARISON_EQUALS_LESS) {
                        ...
                        else if(mtest2[0].number().isInterval() || mtest2[0].number().isRational()) {
                            mthis = m_one;
                            return 0;
                        }
                    } else {
...
cube_eq_stop_iv:
            ...
            if(CHILD(0)[1].contains(x_var)) {
                if((ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) && !CHILD(1).isZero()) {
                    ...
                    if(CHILD(0)[0].representsNonComplex() && CHILD(1).compare(m_zero) == COMPARISON_RESULT_LESS) {
                        ...
                        if(mreq1) marg3->add(*mreq2, ct_comp == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_OR : OPERATION_LOGICAL_AND, true);
                    ...
                    if(CHILD(0)[1].number().isTwo()) {
                        ...
                        if(CHILD(1).representsNegative(true)) {
                            ...
                            if(CHILD(1).isNumber() && !CHILD(1).number().hasRealPart()) {
                                ...
                                default: {
                                    if(CALCULATOR->getRadUnit()) marg /= CALCULATOR->getRadUnit();
                                }
                                }
    ...
    case STRUCT_FUNCTION: {
        if(CHILD(0).function()->id() == FUNCTION_ID_ROOT && VALID_ROOT(CHILD(0))) {
            ...
            mreq2->transform(ct_comp == COMPARISON_NOT_EQUALS ? COMPARISON_GREATER : COMPARISON_EQUALS_LESS, CALCULATOR->getVariableById(VARIABLE_ID_PI));
