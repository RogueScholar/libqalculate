...
bool Unit::useWithPrefixesByDefault() const { return b_use_with_prefixes; }
  ...
  if (ub2 != this) {
    ...
    if (ub2->subtype() == SUBTYPE_COMPOSITE_UNIT) {
      if (u->hasNonlinearRelationTo(ub2) &&
          (((CompositeUnit *)ub2))->containsRelativeTo(u))
        return true;
    }
    return false;
  }
...
bool Unit::hasApproximateRelationTo(
  ...
  if (ub2 != this) {
    ...
    if (ub2->subtype() == SUBTYPE_COMPOSITE_UNIT) {
      if ((((CompositeUnit *)ub2))->containsRelativeTo(u) &&
...
bool Unit::convert(Unit *u, MathStructure &mvalue, MathStructure &mexp) const {
  if (u == this) {
    ...
    if (isCurrency() && u->isCurrency() &&
        ((isBuiltin() && this != CALCULATOR->getUnitById(UNIT_ID_EURO)) ||
         (u->isBuiltin() && u != CALCULATOR->getUnitById(UNIT_ID_EURO)))) {
      int i = 1;
      if (i < 4 && (u == CALCULATOR->getUnitById(UNIT_ID_BYN) ||
                    this == CALCULATOR->getUnitById(UNIT_ID_BYN)))
        i = (i == 3 ? 5 : 4);
      if (i < 2 && (u == CALCULATOR->getUnitById(UNIT_ID_BTC) ||
                    this == CALCULATOR->getUnitById(UNIT_ID_BTC)))
        i = 2;
      if (i < 5 && u->subtype() == SUBTYPE_ALIAS_UNIT) {
        if (i < 2 && ((AliasUnit *)u)->firstBaseUnit() ==
                         CALCULATOR->getUnitById(UNIT_ID_BTC))
          i = 2;
        else if (i < 5 && ((AliasUnit *)u)->firstBaseUnit() !=
                              CALCULATOR->getUnitById(UNIT_ID_EURO))
          i = (i == 4 ? 5 : 3);
        else if (i < 4 && ((AliasUnit *)u)->firstBaseUnit() ==
                              CALCULATOR->getUnitById(UNIT_ID_BYN))
          i = (i == 3 ? 5 : 4);
      }
      if (i < 5 && subtype() == SUBTYPE_ALIAS_UNIT) {
        if (i < 2 && ((AliasUnit *)this)->firstBaseUnit() ==
                         CALCULATOR->getUnitById(UNIT_ID_BTC))
          i = 2;
        else if (i < 5 && ((AliasUnit *)this)->firstBaseUnit() !=
                              CALCULATOR->getUnitById(UNIT_ID_EURO))
          i = (i == 4 ? 5 : 3);
        else if (i < 4 && ((AliasUnit *)this)->firstBaseUnit() ==
                              CALCULATOR->getUnitById(UNIT_ID_BYN))
          i = (i == 3 ? 5 : 4);
      }
...
AliasUnit_Composite::AliasUnit_Composite(Unit *alias, int exp, Prefix *prefix_)
    : AliasUnit("", alias->name(), alias->plural(false), alias->singular(false),
                "", alias, "", exp, "") {
...
string AliasUnit_Composite::print(
  ...
  if (prefixv) {
    str +=
        prefixv->name(short_, use_unicode, can_display_unicode_string_function,
                      can_display_unicode_string_arg);
  }
  str += preferredName(short_, use_unicode, plural_, false,
                       can_display_unicode_string_function,
                       can_display_unicode_string_arg)
             .name;
  return str;
      ...
      if (mstruct[i].isUnit()) {
        ...
        for (size_t i2 = 0; i2 < mstruct.size(); i2++) {
          if (mstruct[i][i2].isUnit()) {
            ...
            add(mstruct[i][i2][0].unit(), mstruct[i][i2][1].number().intValue(),
