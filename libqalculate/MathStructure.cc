...
MathStructure::~MathStructure() { clear(); }
...
bool MathStructure::isMatrix() const {
  if (m_type != STRUCT_VECTOR || SIZE < 1 || CHILD(0).size() == 0)
    return false;
  for (size_t i = 0; i < SIZE; i++) {
    if (!CHILD(i).isVector() ||
...
int contains_interval_var(const MathStructure &m, bool structural_only,
  ...
  if (include_interval_function && m.type() == STRUCT_FUNCTION &&
...
int MathStructure::containsInterval(bool structural_only, bool check_variables,
  ...
  if (include_interval_function && m_type == STRUCT_FUNCTION &&
    ...
    if (check_variables && m_type == STRUCT_VARIABLE) {
      ...
      if (mtype == STRUCT_UNIT) {
        ...
        if (o_function->subtype() == SUBTYPE_USER_FUNCTION ||
          ...
          return -1;
        // (eo.parse_options.angle_unit == ANGLE_UNIT_NONE && (o_function->id()
        // == FUNCTION_ID_ASIN || o_function->id() == FUNCTION_ID_ACOS ||
        // o_function->id() == FUNCTION_ID_ATAN || o_function->id() ==
        ...
        if (o_function->id() == FUNCTION_ID_LOG ||
            ...
            o_function->id() == FUNCTION_ID_LI ||
            o_function->id() == FUNCTION_ID_POLYLOG ||
            o_function->id() == FUNCTION_ID_EI ||
            o_function->id() == FUNCTION_ID_SI ||
            o_function->id() == FUNCTION_ID_CI ||
            o_function->id() == FUNCTION_ID_SHI ||
            o_function->id() == FUNCTION_ID_CHI ||
