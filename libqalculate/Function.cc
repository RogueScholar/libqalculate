...
string MathFunction::printCondition() {
  if (scondition.empty() || last_argdef_index == 0)
    return scondition;
...
int MathFunction::args(const string &argstr, MathStructure &vargs,
      ...
      if (pars == 0 && !in_cit1 && !in_cit2) {
        ...
        if (itmp <= maxargs() || args() < 0) {
          ...
          if (!arg && itmp > argc && args() < 0 &&
              itmp > (int)last_argdef_index && last_argdef_index > 0) {
          ...
          CALCULATOR->error(false,
                            _("Additional arguments for function %s() was "
                              "ignored. Function can only use %s argument(s)."),
  ...
  if (!str.empty()) {
    ...
    if (itmp <= maxargs() || args() < 0) {
      ...
      CALCULATOR->error(false,
                        _("Additional arguments for function %s() was ignored. "
                          "Function can only use %s argument(s)."),
                        name().c_str(), i2s(maxargs()).c_str(), NULL);
    }
  }
...
bool MathFunction::testArgumentCount(int itmp) {
  ...
  if (b) {
    CALCULATOR->error(
        true, _("You need at least %s argument(s) (%s) in function %s()."),
        i2s(minargs()).c_str(), str.c_str(), name().c_str(), NULL);
  } else {
    CALCULATOR->error(true,
                      _("You need at least %s argument(s) in function %s()."),
    ...
    if (it->second && it->first > 0 && it->first <= vargs.size()) {
      if (it->second->type() == ARGUMENT_TYPE_SYMBOLIC &&
        ...
        if (vargs[it->first - 1].isUndefined()) {
          ...
          CALCULATOR->error(false, _("No was unknown variable/symbol found."),
                            NULL);
        }
      }
...
void MathFunction::setDefaultValue(size_t arg_, string value_) {
  if ((int)arg_ > argc && (int)arg_ <= max_argc &&
      (int)default_values.size() >= (int)arg_ - argc) {
...
bool replace_f_interval(MathStructure &mstruct, const EvaluationOptions &eo) {
  if (mstruct.isFunction() && mstruct.function() == CALCULATOR->f_interval &&
      mstruct.size() == 2) {
...
int UserFunction::calculate(MathStructure &mstruct, const MathStructure &vargs,
  ...
  if (args() != 0) {
    ...
    for (int i = 0; i < i_args; i++) {
      if (vargs[i].containsInterval(true) ||
          vargs[i].containsFunction(CALCULATOR->f_interval, true)) {
      ...
      if (i < 24 && (i2 = new_formula.find(svar_o, i4)) != string::npos) {
        ...
        before_find_in_vsubs_set_formula:
          if (i < 24 && (i2 = v_subs[sub_i].find(svar_o, i4)) != string::npos) {
            ...
            i3 = 0;
            if (v_subs[sub_i].length() > i2 + 2 &&
                v_subs[sub_i][i2 + 2] == ID_WRAP_LEFT_CH) {
              if ((i3 = v_subs[sub_i].find(ID_WRAP_RIGHT_CH, i2 + 2)) !=
                  string::npos) {
    ...
    while ((i = str2.find(ID_WRAP_LEFT, i)) != string::npos) {
      ...
      if (!m_temp) {
        CALCULATOR->error(true, _("Internal id %s does not exist."),
                          i2s(id).c_str(), NULL);
        str3 +=
            CALCULATOR->v_undef->preferredInputName(true, false, false, true)
                .name;
      } else {
        str3 += m_temp->print(CALCULATOR->save_printoptions).c_str();
        m_temp->unref();
...
void AngleArgument::parse(MathStructure *mstruct, const string &str,
  ...
  if (po.angle_unit != ANGLE_UNIT_NONE) {
    if (mstruct->contains(CALCULATOR->getRadUnit(), false, true, true) > 0)
      return;
    if (mstruct->contains(CALCULATOR->getDegUnit(), false, true, true) > 0)
      return;
    if (mstruct->contains(CALCULATOR->getGraUnit(), false, true, true) > 0)
      return;
  }
