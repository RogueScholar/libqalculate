...
#define QALCULATE_MINOR_VERSION (7)
...
#define BASE_FLOAT16 -16
#define BASE_FLOAT32 -32
#define BASE_FLOAT64 -64
#define BASE_FLOAT128 -128
...
static const struct PrintOptions {
  ...
  /// Restrict the length of numerators and demonitor as integers in decimal
...
typedef enum {
  ...
  /// Convert to the the least amount of units. Non-SI units is kept (if
  /// optimal), but for conversion only SI units are used.
...
static const struct ParseOptions {
  ...
  /// "x * y").  Default: false
  /**
   * If the limit implicit multiplication mode is activated, the use of
   * implicite multiplication when parsing expressions and displaying results
   * will be limited to avoid confusion. For example, if this mode is not
   * activated and "integrte(5x)" is accidently typed instead of
   * "integrate(5x)", the expression is interpreted as "int(e * e * (5 * x) * gr
   * * t)". If limit implicit multiplication is turned on to mistyped expression
   * would instead show an error telling that "integrte" is not a valid
   * variable, function or unit (unless unknowns is not enabled in which case
   * the result will be "5 'integrate' * x".
   */
  bool limit_implicit_multiplication;
  ...
  bool preserve_format;
  /// Default dataset. Used for object.property syntax without a preceeding data
  /// set. Default: NULL
  DataSet *default_dataset;
  /// Parsing mode. Default: PARSING_MODE_ADAPTIVE
  ParsingMode parsing_mode;
  /// Negative binary numbers uses two's complement representation. All binary
  /// numbers starting with 1 are assumed to be negative. Default: false
  bool twos_complement;
  /// Negative hexadecimal numbers uses two's complement representation. All
...
static const struct EvaluationOptions {
  ...
  bool sync_units;
  /// If units with complex/non-linear relations (ex. degress celsius and
  ...
  /// If a varaible will be isolated to the left side in equations/comparisons
