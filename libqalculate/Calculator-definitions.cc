...
bool Calculator::fetchExchangeRates(int timeout, int n) {
    ...
    if(n <= 0 || n >= 4) {
        ...
        if(res != CURLE_OK) {
            error(true, _("Failed to download exchange rates from %s: %s."), "nbrb.by", error_buffer, NULL);
            FETCH_FAIL_CLEANUP;
            return false;
        }
        if(sbuffer.empty()) {
            error(true, _("Failed to download exchange rates from %s: %s."), "nbrb.by", "Document empty", NULL);
            FETCH_FAIL_CLEANUP;
            return false;
        }
