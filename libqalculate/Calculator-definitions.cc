...
time_t Calculator::getExchangeRatesTime(int index) {
  ...
  if (index < 1) {
    ...
    for (int i = 1; i < index; i++) {
      if (i > 2 && priv->exchange_rates_time2[i - 2] < extime)
        extime = priv->exchange_rates_time2[i - 2];
  ...
  if (index > 2)
    return priv->exchange_rates_time2[index - 2];
...
bool Calculator::fetchExchangeRates(int timeout, int n) {
  ...
  if (n <= 0 || n >= 4) {
    ...
    if (res != CURLE_OK) {
      error(true, _("Failed to download exchange rates from %s: %s."),
            "nbrb.by", error_buffer, NULL);
      FETCH_FAIL_CLEANUP;
      return false;
    }
    if (sbuffer.empty()) {
      error(true, _("Failed to download exchange rates from %s: %s."),
            "nbrb.by", "Document empty", NULL);
      FETCH_FAIL_CLEANUP;
      return false;
    }
...
bool Calculator::checkExchangeRatesDate(unsigned int n_days, bool force_check,
  ...
  for (int i = 1; i < n; i++) {
    if (i > 2 && priv->exchange_rates_check_time2[i - 2] < cextime)
      cextime = priv->exchange_rates_check_time2[i - 2];
    else if (i <= 2 && (i != 2 || n != 4) &&
  ...
  for (int i = 0; i < n; i++) {
    if (i <= 2 && (i != 2 || n != 4))
      ...
      time(&priv->exchange_rates_check_time2[i - 2]);
  }
  if (send_warning)
    error(
        false,
