...
bool Number::lambertW(const Number &k) {
    ...
    if(isInterval(false)) {
        if(hasImaginaryPart() && i_value->isInterval()) {
            ...
            if(hasImaginaryPart()) CALCULATOR->error(false, _("%s() lacks proper support interval arithmetic."), CALCULATOR->f_lambert_w->name().c_str(), NULL);
            return setInterval(nr_l, nr_u, true);
        }
    }
    ...
    if(k.isZero() || k.isMinusOne()) {
        if(ip1 && !k.isOne()) {
            ...
            if(nr <= nr_half || (k.isZero() && nr <= Number(5, 8))) {
                if(k.isZero()) {
                    Number ndiv(z);
                    if(!ndiv.multiply(2) || !ndiv.add(1) || !ndiv.multiply(Number("0.827184")) || !ndiv.add(2) || !multiply(Number("7.061302897")) || !add(Number("0.1237166")) || !multiply(Number("0.35173371")) || !divide(ndiv)) {
                        CALCULATOR->endTemporaryStopIntervalArithmetic();
                        set(nr_bak);
                        return false;
                    }
                    ...
                    set(i2);
                    if(!multiply(z) || !add(i3) || !multiply(i1) || !divide(i4) || !negate()) {
                        CALCULATOR->endTemporaryStopIntervalArithmetic();
                        set(nr_bak);
                        return false;
                    }
