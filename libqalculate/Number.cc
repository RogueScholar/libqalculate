...
void Number::set(string number, const ParseOptions &po) {
  ...
  for (size_t index = 0; index < number.size(); index++) {
    if (number[index] >= '0' && ((base >= 10 && number[index] <= '9') ||
      ...
      minus = !minus;
    } else if (number[index] == 'i' || (CALCULATOR && number[index] == 'j' &&
                                        CALCULATOR->v_i->hasName("j"))) {
      b_cplx = true;
  ...
  if (i_unc <= 0 &&
    ...
    if (i_unc > 0) {
      ...
      mpq_clear(unc);
    } else if (po.read_precision == ALWAYS_READ_PRECISION ||
               (in_decimals &&
...
int Number::equalsApproximately(const Number &o, int prec) const {
  ...
  if (prec == EQUALS_PRECISION_LOWEST) {
    prec = PRECISION;
    if (i_precision >= 0 && i_precision < prec)
      prec = i_precision;
    if (o.precision() >= 0 && o.precision() < prec)
      prec = o.precision();
...
bool Number::zeta() {
  ...
  if (overflow) {
    CALCULATOR->error(
        true, _("Cannot handle an argument (s) that large for Riemann Zeta."),
        NULL);
    return false;
  }
...
bool Number::gamma() {
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    if (mpfr_sgn(fl_value) > 0) {
      ...
      if (mpfr_cmp(fl_value, fu_value) > 0)
        mpfr_swap(fl_value, fu_value);
      if (nr_bak.isInterval() && nr_bak.precision(1) <= PRECISION + 20)
        CALCULATOR->error(false,
                          _("%s() lacks proper support interval arithmetic."),
                          CALCULATOR->f_gamma->name().c_str(), NULL);
    ...
    if (hasRealPart()) {
      if (isInterval(false)) {
        ...
        CALCULATOR->error(
            ...
            CALCULATOR->getFunctionById(FUNCTION_ID_ERF)->name().c_str(), NULL);
      ...
      if (n_type == NUMBER_TYPE_FLOAT)
        yprev.setInternal(fl_value);
      else
        yprev.setInternal(r_value);
      if (i_value)
        yprevi.set(*i_value);
      while (true) {
        den = k;
        ...
        xpow++;
        num = x;
        if (CALCULATOR->aborted() || !kfac.multiply(k) || !den.multiply(kfac) ||
            !den.add(kfac) || !num.raise(xpow) || !num.divide(den) ||
            (k.isOdd() && !num.negate()) || !add(num) || includesInfinity()) {
          CALCULATOR->endTemporaryStopIntervalArithmetic();
          set(nr_bak);
          return false;
        }
      ...
      if (isInterval(false)) {
        ...
        if (i_value->isInterval()) {
          ...
          if (isInterval(true)) {
            ...
            nr_l2.intervalToMidValue();
            nr_u2.intervalToMidValue();
            if (!nr_l.erfi() || !nr_u.erfi() || !nr_l2.erfi() || !nr_u2.erfi())
              return false;
            nr_u.setInterval(nr_u, nr_u2);
            nr_l.setInterval(nr_l, nr_l2);
          } else {
        ...
        CALCULATOR->error(
            false, _("%s() lacks proper support interval arithmetic."),
            CALCULATOR->getFunctionById(FUNCTION_ID_ERFI)->name().c_str(),
            NULL);
        return setInterval(nr_l, nr_u, true);
      }
      Number nr_bak(*this);
...
bool Number::airy() {
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    if (nr_bak.isInterval() && nr_bak.precision(1) <= PRECISION + 20)
      CALCULATOR->error(false,
                        _("%s() lacks proper support interval arithmetic."),
                        CALCULATOR->f_airy->name().c_str(), NULL);
  }
  if (!testFloatResult()) {
    set(nr_bak);
    return false;
...
bool Number::besselj(const Number &o) {
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    if (nr_bak.isInterval() && nr_bak.precision(1) <= PRECISION + 20)
      CALCULATOR->error(false,
                        _("%s() lacks proper support interval arithmetic."),
                        CALCULATOR->f_besselj->name().c_str(), NULL);
  }
  if (!testFloatResult()) {
    set(nr_bak);
    return false;
...
bool Number::bessely(const Number &o) {
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    if (nr_bak.isInterval() && nr_bak.precision(1) <= PRECISION + 20)
      CALCULATOR->error(false,
                        _("%s() lacks proper support interval arithmetic."),
                        CALCULATOR->f_bessely->name().c_str(), NULL);
  }
  if (!testFloatResult()) {
    set(nr_bak);
    return false;
...
bool Number::lambertW(const Number &k) {
  ...
  if (isInterval(false)) {
    if (hasImaginaryPart() && i_value->isInterval()) {
      ...
      if (isInterval(true)) {
        ...
        CALCULATOR->error(false,
                          _("%s() lacks proper support interval arithmetic."),
                          CALCULATOR->f_lambert_w->name().c_str(), NULL);
        return setInterval(nr_l, nr_u, true);
      } else {
        nr_l = realPart();
        nr_u = realPart();
  ...
  if (k.isZero() || k.isMinusOne()) {
      ...
      if (nr <= nr_half || (k.isZero() && nr <= Number(5, 8))) {
        if (k.isZero()) {
          ...
          if (!ndiv.multiply(2) || !ndiv.add(1) ||
              ...
              !multiply(Number("7.061302897")) || !add(Number("0.1237166")) ||
              !multiply(Number("0.35173371")) || !divide(ndiv)) {
            CALCULATOR->endTemporaryStopIntervalArithmetic();
            set(nr_bak);
            return false;
          }
          ...
          set(i2);
          if (!multiply(z) || !add(i3) || !multiply(i1) || !divide(i4) ||
              !negate()) {
            CALCULATOR->endTemporaryStopIntervalArithmetic();
            set(nr_bak);
            return false;
    ...
    if (ip1 && (!k.isMinusOne() || z.imaginaryPartIsPositive())) {
      ...
      if (nr <= 1) {
        ...
        set(p);
        if ((!k.isZero() && !negate()) || !add(-1) || !add(p2) || !add(p3)) {
          CALCULATOR->endTemporaryStopIntervalArithmetic();
          set(nr_bak);
          return false;
        }
  ...
  if (n_type == NUMBER_TYPE_FLOAT)
    wprev.setInternal(fl_value);
  else
    wprev.setInternal(r_value);
  if (i_value)
    wprevi.set(*i_value);
  while (true) {
    wexp.set(*this);
    ...
    if (!wexpw.subtract(z) || !wexpw_dd.multiply(wexpw) ||
        !wexpw.multiply(wexpw_d) || !wexpw.multiply(-2) || !wexpw_d.square() ||
        !wexpw_d.multiply(2) || !wexpw_d.subtract(wexpw_dd) ||
        !wexpw.divide(wexpw_d) || !add(wexpw)) {
      CALCULATOR->endTemporaryStopIntervalArithmetic();
      set(nr_bak);
      return false;
    }
    ...
    if (wtest < wprec && wtesti < wprec) {
      ...
      if (wtesti.isZero() && hasImaginaryPart() && !b_real) {
        wtesti++;
...
bool Number::lambertW() {
  ...
  if (cmp == 0) {
    ...
    mpfr_set(fu_value, fl_value, MPFR_RNDN);
    if (CREATE_INTERVAL) {
      if (mpfr_zero_p(testXW)) {
        mpfr_nextbelow(fl_value);
        mpfr_nextabove(fu_value);
      } else {
        mpfr_mul(testXW, testXW, fl_value, MPFR_RNDA);
        mpfr_abs(testXW, testXW, MPFR_RNDU);
        mpfr_add(fu_value, fu_value, testXW, MPFR_RNDU);
        mpfr_sub(fl_value, fl_value, testXW, MPFR_RNDD);
      }
    } else {
      if (i_precision < 0 ||
          i_precision > FROM_BIT_PRECISION(BIT_PRECISION - 30))
...
bool Number::igamma(const Number &o) {
  ...
  if (!CREATE_INTERVAL && !isInterval() && !o_float.isInterval()) {
    ...
    if (!o.isGreaterThanOrEqualTo(1) && !nr_bak.isGreaterThan(2) &&
      ...
      CALCULATOR->error(false,
                        _("%s() lacks proper support interval arithmetic."),
                        CALCULATOR->f_igamma->name().c_str(), NULL);
  }
  if (!testFloatResult()) {
    set(nr_bak);
    return false;
