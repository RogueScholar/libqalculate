...
string Number::print(const PrintOptions &po,
  ...
  integer_rerun:
    ...
    if (!is_interval) {
      ...
      mpfr_inits2(mpfr_get_prec(fl_value), f_mid, vl, vu, f_logl, f_base,
                  f_log_base, NULL);
    ...
    float_interval_prec_rerun:
      ...
      if (po.use_max_decimals && po.max_decimals >= 0) {
        ...
        if (!po.preserve_format && base == 10) {
          expo = i_log;
          if (po.min_exp == EXP_PRECISION ||
              (po.min_exp == 0 && expo > 1000000L)) {
            long int precexp = i_precision_base;
            if (precision < 8 && precexp > precision + 2)
              precexp = precision + 2;
            else if (precexp > precision + 3)
              precexp = precision + 3;
            if ((expo > 0 && expo < precexp) ||
                (expo < 0 && expo > -PRECISION)) {
        ...
        if (i_log - expo + po.max_decimals < i_precision_base)
          i_precision_base = i_log - expo + po.max_decimals;
      }
      ...
      for (size_t i = 0; i < str_l.size(); i++) {
        if (str_l[i] != str_u[i]) {
          ...
          if (i == 0 || i_precision_base <= 0) {
            ...
            if (mpfr_cmp_ui(f_mid, 1) >= 0) {
              i_precision_base = 0;
              break;
            }
            i_precision_base = -1;
          } else if (i - 1 >= (size_t)i_precision_base)
            i_precision_base--;
          else
            i_precision_base = i - 1;
          goto float_interval_prec_rerun;
      ...
      if (i_precision_base < precision_base)
      ...
      if (precision_base <= 1) {
        ...
        if (mpfr_cmp_abs(f_mid, fu_value) > 0 ||
            mpfr_cmp_abs(f_mid, fl_value) < 0) {
  ...
  rational_rerun:
    ...
    if (!rerun && !approx && !exact && num_sign == 0 && expo <= 0 &&
        min_decimals > 0 && l10 + expo < min_decimals) {
    ...
    if (l10 > 0) {
      ...
      if ((!exact || approx) && !infinite_series && po.show_ending_zeroes &&
          (int)str.length() - precision_base - 1 < l2) {
        l2 = str.length() - precision_base - 1;
        if (po.use_max_decimals && po.max_decimals >= 0 &&
      ...
      if (l2 > 0 && !infinite_series) {
        if (min_decimals > 0 &&
          ...
          if (decimals - min_decimals < l2)
            l2 = decimals - min_decimals;
          if (approx && (int)str.length() - precision - 1 > l2)
            l2 = str.length() - precision_base - 1;
        }
        if (l2 > 0)
          str = str.substr(0, str.length() - l2);
