...
string Number::print(const PrintOptions &po,
  ...
  if ((po.base == BASE_SEXAGESIMAL || po.base == BASE_TIME) && isReal()) {
    ...
    if (!nr2.isZero() || po.base == BASE_SEXAGESIMAL) {
      ...
      // if left over fractional part != 0, output is approximate, round upwards
      // if >= 0.5
      nr2.frac();
      if (!nr2.isZero()) {
        if (po.is_approximate)
  ...
  if (isInteger()) {
    ...
    // condition: 	number of digits > 1000 and number is approximate, or base =
    // 10 and scientific notation and not unrestricted fraction format
