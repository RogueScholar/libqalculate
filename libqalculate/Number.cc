    ...
    if (mpfr_cmp(yprev, wprec) < 0) {
      ...
      if (CREATE_INTERVAL) {
        ...
        if (nr_bak > 50 || nr_bak < -50) {
          if (i_precision < 0 ||
              i_precision > FROM_BIT_PRECISION(BIT_PRECISION - 20))
            i_precision = FROM_BIT_PRECISION(BIT_PRECISION - 20);
        } else {
          if (i_precision < 0 ||
...
bool Number::lambertW(const Number &k) {
  ...
  while (true) {
    ...
    if (!wexpw.subtract(z) || !wexpw_dd.multiply(wexpw) ||
        ...
        !wexpw.divide(wexpw_d) || !add(wexpw) || includesInfinity()) {
      CALCULATOR->endTemporaryEnableIntervalArithmetic();
      set(nr_bak);
      return false;
    }
...
bool Number::polylog(const Number &o) {
  ...
  if (!o.isTwo() || !isLessThanOrEqualTo(1)) {
    ...
    while (true) {
      ...
      kpow = k;
      if (i == 0) {
        if (CALCULATOR->aborted() || !kfac.multiply(k) || !kpow.negate() ||
            !kpow.add(o) || !kpow.zeta() || !xpow.ln() || !xpow.raise(k) ||
            !xpow.multiply(kpow) || !xpow.divide(kfac) || !add(xpow) ||
            includesInfinity()) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          set(nr_bak);
          return false;
        ...
        kpow = k;
        if (CALCULATOR->aborted() || !kpow.raise(oneg) || !xpow.raise(k) ||
            (i == 2 && !xpow.recip()) || !xpow.multiply(kpow) || !add(xpow) ||
            includesInfinity()) {
          CALCULATOR->endTemporaryEnableIntervalArithmetic();
          set(nr_bak);
          return false;
      ...
      if (ytest < wprec && ytesti < wprec) {
        ...
        if (ytesti.isZero() && hasImaginaryPart()) {
          ytesti++;
          ytesti.setToFloatingPoint();
          mpfr_nextabove(ytesti.internalUpperFloat());
          ytesti--;
...
bool Number::expint() {
  ...
  if (hasImaginaryPart()) {
    ...
    if (isInterval(false)) {
      ...
      CALCULATOR->error(
          false, _("%s() lacks proper support interval arithmetic."),
          CALCULATOR->getFunctionById(FUNCTION_ID_EI)->name().c_str(), NULL);
      return setInterval(nr_l, nr_u, true);
    }
    Number nr_euler;
    nr_euler.euler();
    ...
    while (true) {
      xpow = x;
      if (CALCULATOR->aborted() || !kfac.multiply(k) || !xpow.raise(k) ||
          !xpow.divide(kfac) || !xpow.divide(k) || !add(xpow) ||
          includesInfinity()) {
        CALCULATOR->endTemporaryEnableIntervalArithmetic();
        set(nr_bak);
        return false;
