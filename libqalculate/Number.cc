  ...
  if (po.digit_grouping != DIGIT_GROUPING_NONE &&
    ...
    if (po.digit_grouping == DIGIT_GROUPING_LOCALE &&
      ...
      if (CALCULATOR->local_digit_group_format[i_format] == CHAR_MAX)
        return;
      if (CALCULATOR->local_digit_group_format[i_format] > 0)
    ...
    if (po.digit_grouping == DIGIT_GROUPING_LOCALE || i > group_size + 1) {
      while (i > group_size) {
        ...
        if (po.digit_grouping == DIGIT_GROUPING_LOCALE &&
            CALCULATOR->local_digit_group_format.size() - 1 > i_format) {
          i_format++;
          if (CALCULATOR->local_digit_group_format[i_format] == CHAR_MAX)
            break;
          if (CALCULATOR->local_digit_group_format[i_format] > 0)
...
void Number::set(string number, const ParseOptions &po) {
  ...
  if (po.base == BASE_BIJECTIVE_26) {
    ...
    for (size_t i = 0; i < str.length();) {
      if (!(str[i] >= 'a' && str[i] <= 'z') &&
        ...
        while (i + n < str.length() && str[i + n] < 0 &&
      ...
      for (size_t i = 0; i < number.length(); i++) {
        ...
        if (c >= 0) {
          if (abs_base <= c && !abs_base.isFraction()) {
            ...
            // but use anyway
            CALCULATOR->error(false,
                              _("Digit \'%s\' is too high for number base."),
                              number.substr(i, 1).c_str(), NULL);
          }
          digits.push_back(c);
        }
      }
        ...
        if (number[i] == '\\' && i < number.length() - 1) {
          ...
          if (digits.empty() && number[i] == (char)-30 &&
            ...
            if (abs_base.isLessThanOrEqualTo(nrd)) {
              CALCULATOR->error(
                  false, _("Digit \'%s\' is too high for number base."),
                  number.substr(i_prev, i - i_prev + 1).c_str(), NULL);
            }
  ...
  bool b_twos =
      (po.twos_complement && po.base == 2 && number.length() > 1 &&
       number[0] == '1') ||
      (po.hexadecimal_twos_complement && po.base == 16 && number.length() > 0 &&
  ...
  bool numbers_started = false, minus = false, in_decimals = false,
       b_cplx = false, had_nonzero = false;
  for (size_t index = 0; index < number.size(); index++) {
    if (number[index] >= '0' && ((base >= 10 && number[index] <= '9') ||
      ...
      if (number[index] != (b_twos ? '0' + (base - 1) : '0')) {
        ...
        // ignore zeroes at the beginning of the number when determining the
      ...
      if (in_decimals)
        CALCULATOR->error(false, "Misplaced decimal separator ignored", NULL);
      else
        in_decimals = true;
    } else if (number[index] == ':') {
      // sexagesimal number
      ...
      while (index + 1 < number.length() && number[index + 1] < 0 &&
...
bool Number::root(const Number &o) {
  ...
  /*if(o.isEven() && (!isReal() || isNegative())) {
      ...
      while(o_odd_factor.isEven() && !o_odd_factor.isTwo()) {
              if(!o_odd_factor.multiply(nr_half) || !o_even_factor.multiply(2))
  return false; if(CALCULATOR->aborted()) return false;
      }
...
bool Number::trunc() {
  ...
  if (n_type == NUMBER_TYPE_RATIONAL) {
    ...
    if (mpfr_sgn(fu_value) <= 0)
      ...
      mpz_set_ui(mpq_numref(r_value), 0);
    n_type = NUMBER_TYPE_RATIONAL;
    mpfr_clears(fl_value, fu_value, NULL);
  }
  return true;
...
bool Number::zeta() {
  ...
  if (!CREATE_INTERVAL && !isInterval()) {
    ...
    if (b_iverror)
      CALCULATOR->error(
          false, _("%s() lacks proper support interval arithmetic."),
...
bool Number::bernoulli() {
  ...
  case 24: {
    set(Number("-236364091"), true);
    divide(2730);
    return true;
  }
  case 26: {
    set(Number("8553103"), true);
    divide(6);
    return true;
  }
  case 28: {
    set(Number("-23749461029"), true);
    divide(870);
    return true;
  }
  case 30: {
    set(Number("8615841276005"), true);
    divide(14322);
    return true;
  }
  case 32: {
    set(Number("-7709321041217"), true);
    divide(510);
    return true;
  }
  case 34: {
    set(Number("2577687858367"), true);
    divide(6);
    return true;
  }
  case 36: {
    set(Number("-26315271553053477373"), true);
    divide(Number("1919190"));
    return true;
  }
  case 38: {
    set(Number("2929993913841559"), true);
    divide(6);
    return true;
  }
  case 40: {
    set(Number("-261082718496449122051"), true);
    divide(13530);
    return true;
  }
  case 42: {
    set(Number("1520097643918070802691"), true);
    divide(1806);
    return true;
  }
  case 44: {
    set(Number("-27833269579301024235023"), true);
    divide(690);
    return true;
  }
  case 46: {
    set(Number("596451111593912163277961"), true);
    divide(282);
    return true;
  }
  case 48: {
    set(Number("-5609403368997817686249127547"), true);
    divide(46410L);
    return true;
  }
  case 50: {
    set(Number("495057205241079648212477525"), true);
    divide(66);
    return true;
  }
  case 52: {
    set(Number("-801165718135489957347924991853"), true);
    divide(1590);
    return true;
  }
  case 54: {
    set(Number("29149963634884862421418123812691"), true);
    divide(798);
    return true;
  }
  case 56: {
    set(Number("-2479392929313226753685415739663229"), true);
    divide(870);
    return true;
  }
  case 58: {
    set(Number("84483613348880041862046775994036021"), true);
    divide(354);
    return true;
  }
  case 60: {
    set(Number("-1215233140483755572040304994079820246041491"), true);
    divide(Number("56786730"));
    return true;
  }
  case 62: {
    set(Number("12300585434086858541953039857403386151"), true);
    divide(6);
    return true;
  }
  case 64: {
    set(Number("-106783830147866529886385444979142647942017"), true);
    divide(510);
    return true;
  }
  case 66: {
    set(Number("1472600022126335654051619428551932342241899101"), true);
    divide(64722L);
    return true;
  }
  case 68: {
    set(Number("-78773130858718728141909149208474606244347001"), true);
    divide(30);
    return true;
  }
  case 70: {
    set(Number("1505381347333367003803076567377857208511438160235"), true);
    divide(4686);
    return true;
  }
  case 72: {
    set(Number("-5827954961669944110438277244641067365282488301844260429"),
        true);
    divide(Number("140100870"));
    return true;
  }
  case 74: {
    set(Number("34152417289221168014330073731472635186688307783087"), true);
    divide(6);
    return true;
  }
  case 76: {
    set(Number("-24655088825935372707687196040585199904365267828865801"), true);
    divide(30);
    return true;
  }
  case 78: {
    set(Number("414846365575400828295179035549542073492199375372400483487"),
        true);
    divide(3318);
    return true;
  }
  ...
  case 82: {
    set(Number("1677014149185145836823154509786269900207736027570253414881613"),
        true);
    divide(498);
    return true;
  }
  case 84: {
    set(Number("-20245761959352903602311311601117310099899173911980908772810839"
               "32477"),
        true);
    divide(Number("3404310"));
    return true;
  }
  case 86: {
    set(Number(
  ...
  case 88: {
    set(Number("-13114264886740175079955114240193118433457502755720286442969198"
               "90574047"),
        true);
    divide(61410);
    return true;
  ...
  case 90: {
    set(Number("117905727902108279988412335124921508377525494966964711623154521"
               "5727922535"),
        true);
    divide(272118L);
    return true;
  ...
  case 92: {
    set(Number("-12955859482075375279894278285385767496593414837194351430233163"
               "26829946247"),
        true);
    divide(1410);
    return true;
  ...
  case 94: {
    set(Number("122081380657974446960730167941320120395850841520269662143621510"
               "5284649447"),
        true);
    divide(6);
    return true;
  ...
  case 96: {
    set(Number("-21160044959726651309759772810982423367304395438906023415063873"
               "3420050668349987259"),
        true);
    divide(Number("4501770"));
    return true;
  }
  case 98: {
    set(Number("679082606729054956240511175464036056073421957285044875090739612"
               "49992947058239"),
        true);
    divide(6);
    return true;
  ...
  case 100: {
    set(Number("-94598037819122125295227433069493721872702841533066936133385696"
               "204311395415197247711"),
        true);
    divide(33330);
    return true;
  }
  case 102: {
    set(Number("320401941086090707824302078211624177549181719715271745067900250"
               "1086861530836678158791"),
        true);
    divide(4326);
    return true;
  }
  case 104: {
    set(Number("-31953363136383001128710335279617427467118960607827273832710347"
               "0162849568365549721224053"),
        true);
    divide(1590);
    return true;
  }
  case 106: {
    set(Number("363739031726174144081518201515934271692312986405816900389308163"
               "78281879873386202346572901"),
        true);
    divide(642);
    return true;
  }
  case 108: {
    set(Number("-34693422478478287895520886593238525413997667857604911468700058"
               "91371501266319724897592306597338057"),
        true);
    divide(Number("209191710"));
    return true;
  }
  case 110: {
    set(Number("764599294048474289224813424672434750052875241341230790668359387"
               "0759797606269585779977930217515"),
        true);
    divide(1518);
    return true;
  }
  case 112: {
    set(Number("-26508796021550997133525972146851620144431514991925098964517884"
               "27680966756514875515366781203552600109"),
        true);
    divide(Number("1671270"));
    return true;
  }
  case 114: {
    set(Number("217378323193691633333107610866529914757211566790908313608061101"
               "1493360548423459 3650904188618562649"),
        true);
    divide(42);
    return true;
  }
  case 116: {
    set(Number("-30955391657184297691251345803384141686900412806432984424550404"
               "5721008957524571 968271388199595754752259"),
        true);
    divide(1770);
    return true;
  }
  case 118: {
    set(Number("366963119969713111534947151585585006684606361080699204301059440"
               "6764144850458064 61889371776354517095799"),
        true);
    divide(6);
    return true;
  }
  case 120: {
    set(Number("-51507486535079109061843996857849983274095170353262675213092869"
               "1671992974749229 85358811329367077682677803282070131"),
        true);
    divide(Number("2328255930"));
    return true;
  }
  case 122: {
    set(Number("496336660792625819125326374759907574387227903110601397703093117"
               "9315068321410043 1329033113678098037968564431"),
        true);
    divide(6);
    return true;
  }
  case 124: {
    set(Number("-95876775334247128750774903107542444620578830013297336819553512"
               "7293585933544359 44413631943610268472689094609001"),
        true);
    divide(30);
    return true;
  }
  case 126: {
    set(Number("555633028194927485061632440891895138052556730712674724679678230"
               "4333594286400508 981287241419934529638692081513802696639"),
        true);
    divide(Number("4357878"));
    return true;
  }
  case 128: {
    set(Number("-26775470774254808288695440558528239477929145959255174062997868"
               "6063357792734863 530145362663093519862048495908453718017"),
        true);
    divide(510);
    return true;
  }
  case 130: {
    set(Number("192821517513613091564529952227159643530761101016472845878373302"
               "0528548622403504 078595174411693893882739334735142562418015"),
        true);
    divide(8646);
    return true;
  }
  ...
  while (k < m) {
    B_k = nr_k;
    if (CALCULATOR->aborted() || !bin.binomial(nr_m, nr_k) ||
        !B_k.bernoulli() || !B_k.divide(m - k + 1) || !B_k.multiply(bin) ||
        !v.add(B_k))
      return false;
    k++;
    nr_k++;
  ...
  if (isInterval(false)) {
    if (hasImaginaryPart() && i_value->isInterval()) {
      ...
      if (k.isZero()) {
        ...
        mexpm1.exp();
        mexpm1.negate();
        if (COMPARISON_MIGHT_BE_EQUAL(compare(mexpm1)))
...
bool Number::igamma(const Number &o) {
  ...
  return false;
#else
  if (!isReal() || (!o.isNonZero() && !isNonZero()))
    return false;
  Number nr_bak(*this);
  if (!setToFloatingPoint())
    return false;
...
string Number::print(const PrintOptions &po,
  ...
  if (po.base == BASE_CUSTOM)
    base = CALCULATOR->customOutputBase().intValue();
           ...
           po.base != BASE_TIME)
    base = 10;
  else if (po.base > 36 && po.base != BASE_SEXAGESIMAL)
    base = 36;
  else
    base = po.base;
  if (po.base == BASE_ROMAN_NUMERALS) {
  ...
  if (b_approx && i_precision >= 0 &&
    ...
    precision = i_precision;
           ...
           FROM_BIT_PRECISION(BIT_PRECISION) > precision)
    precision = FROM_BIT_PRECISION(BIT_PRECISION);
  else if (b_approx && i_precision < 0 && po.preserve_format &&
           FROM_BIT_PRECISION(BIT_PRECISION) - 1 > precision)
    precision = FROM_BIT_PRECISION(BIT_PRECISION) - 1;
  if (po.restrict_to_parent_precision && ips.parent_precision >= 0 &&
      ips.parent_precision < precision)
    precision = ips.parent_precision;
  long int precision_base = precision;
  ...
  if ((i_precision < 0 && FROM_BIT_PRECISION(BIT_PRECISION) > precision) ||
      i_precision > precision) {
    if (i_precision < 0)
  ...
  integer_rerun:
    ...
    if (ips.minus)
      *ips.minus = neg;
    str = format_number_string(mpz_str, base, po.base_display,
    ...
    if (!is_interval) {
      ...
      mpfr_t vl, vu, f_logl, f_base, f_log_base, f_logu;
      ...
      if (mpfr_sgn(fl_value) != mpfr_sgn(fu_value)) {
        ...
        mpfr_clears(vu, vl, f_logl, f_logu, f_base, f_log_base, NULL);
        mpq_clear(base_half);
        if (ilogu < ilogl)
          ilogl = ilogu;
        if (ilogl <= 1) {
    ...
    float_interval_prec_rerun:
      ...
      if (base != 10 &&
          (mpfr_get_exp(vu) > 100000L || mpfr_get_exp(vu) < -100000L)) {
        if (base == 2) {
          mpfr_log2(f_logl, vu, MPFR_RNDN);
        } else {
        ...
        mpz_t z_exp;
        mpz_init(z_exp);
        mpfr_get_z(z_exp, f_logl, MPFR_RNDN);
        if (mpz_sgn(z_exp) < 0) {
          mpz_neg(z_exp, z_exp);
          neg_bexp = true;
        }
        ...
        mpfr_ui_pow(f_logu, base, f_logl, MPFR_RNDU);
        mpfr_ui_pow(f_logl, base, f_logl, MPFR_RNDD);
      ...
      if (i_log_mod != 0)
        mpfr_add_si(f_logl, f_logl, i_log_mod, MPFR_RNDN);
      mpfr_floor(f_logl, f_logl);
      ...
      if (po.use_max_decimals && po.max_decimals >= 0 && po.use_max_decimals &&
          i_log - expo + po.max_decimals + 1 < i_precision_base) {
        precision_base = i_log - expo + po.max_decimals + 1;
      } else {
        if (po.use_min_decimals && po.min_decimals > 0 &&
            i_log - expo + po.min_decimals + 1 > precision_base) {
      ...
      mpfr_sub_si(f_logu, f_logl, precision_base - 1, MPFR_RNDU);
      mpfr_sub_si(f_logl, f_logl, precision_base - 1, MPFR_RNDD);
      mpfr_pow(f_logl, f_base, f_logl, MPFR_RNDD);
      mpfr_pow(f_logu, f_base, f_logu, MPFR_RNDU);
      mpfr_div(vl, vl, f_logu, MPFR_RNDD);
      mpfr_div(vu, vu, f_logl, MPFR_RNDU);
      if (mpfr_cmp(vl, vu) > 0)
        mpfr_swap(vl, vu);
      ...
      mpfr_clears(vl, vu, f_logl, f_logu, f_base, f_log_base, NULL);
      ...
      if (!str_bexp.empty()) {
        PrintOptions po2 = po;
        po2.binary_bits = 0;
        str = format_number_string(str, base, po.base_display,
  ...
  float_rerun:
    ...
    if (base != 10 &&
        (mpfr_get_exp(f_mid) > 100000L || mpfr_get_exp(f_mid) < -100000L)) {
      bool b_neg = mpfr_sgn(f_mid) < 0;
      if (b_neg)
        mpfr_neg(f_mid, f_mid, MPFR_RNDN);
      if (base == 2) {
    ...
    if (!rerun && i_precision_base > precision_base && min_decimals > 0 &&
        (po.interval_display != INTERVAL_DISPLAY_PLUSMINUS || !is_interval)) {
    ...
    mpfr_sub_si(
        f_log, f_log,
        ((use_max_idp || po.interval_display != INTERVAL_DISPLAY_PLUSMINUS ||
          !is_interval) &&
         po.use_max_decimals && po.max_decimals >= 0 &&
      ...
      if (!rerun) {
        if (str_unc.empty() && po.use_max_decimals && po.max_decimals >= 0) {
          ...
          if (precision <= 0) {
            PrintOptions po2 = po;
            po2.interval_display = INTERVAL_DISPLAY_INTERVAL;
            mpfr_clears(f_mid, f_lunc, f_unc, v, f_log, f_base, f_log_base,
                        NULL);
            mpz_clear(ivalue);
            return print(po2, ips);
          }
    ...
    if (!str_bexp.empty()) {
      PrintOptions po2 = po;
      po2.binary_bits = 0;
      str = format_number_string(str, base, po.base_display, !ips.minus && neg,
                                 true, po2);
      if (!str_unc.empty())
        str_unc = format_number_string(str_unc, base, po.base_display, false,
                                       true, po2);
    } else {
      str = format_number_string(str, base, po.base_display, !ips.minus && neg,
                                 true, po);
  ...
  rational_rerun:
    ...
    if (po.is_approximate && !exact && !infinite_series)
      *po.is_approximate = true;
    ...
    if (l10 > 0) {
      ...
      int decimals = str.length() - l10 - 1;
      if ((!exact || approx) && !infinite_series && po.show_ending_zeroes &&
    ...
    if (!exact && str == "0" && po.show_ending_zeroes && po.use_max_decimals &&
