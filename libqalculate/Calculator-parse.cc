...
// determine if character is not a numerical digit in a specific number base
...
bool is_not_number(char c, int base) {
  ...
  if (base == -12)
    return c != 'E' && c != 'X' && c != 'A' && c != 'B' && c != 'a' && c != 'b';
  if (base <= 10)
    return true;
...
// case sensitive string comparison; compares whole name with str from str_index
// to str_index + name_length
bool compare_name(const string &name, const string &str,
  ...
  // number base uses digits other than 0-9, check that at least one non-digit
  // is used
  if (base < 2 || base > 10) {
...
// case insensitive string comparison; compares whole name with str from
// str_index to str_index + name_length
...
string Calculator::parseComments(string &str, const ParseOptions &po,
                                 bool *double_tag) {

  if (str.length() <= 1 || po.base == BASE_UNICODE ||
  ...
  if (str[0] == '#') {
    ...
    if (from_str.length() == 1 || i == 0 ||
        !getActiveExpressionItem(i == string::npos ? from_str
                                                   : from_str.substr(0, i))) {
  ...
  if (i4 != string::npos) {
    ...
    if ((i2 == i4 && i3 == i4) ||
        !getActiveExpressionItem(i2 == string::npos
                                     ? from_str.substr(i3)
                                     : from_str.substr(i3, i2 - i3 + 1))) {
      ...
      if ((i2 == i && i3 == i) ||
        ...
        str = str.substr(0, i);
        if (to_str.length() > 1 && to_str[1] == '#')
          to_str = to_str.substr(1);
        remove_blank_ends(to_str);
        return to_str;
      }
    }
...
void Calculator::parseSigns(string &str,
  ...
  while (true) {
    ...
    // perform replacement; interpret as addition if previous character is a
    // numeral digit
    if (str[ui + 2] == -98)
  ...
  if (convert_to_internal_representation) {
    ...
    // replace operators with multiple chars with internal single character
    // version
...
bool Calculator::parseNumber(MathStructure *mstruct, string str,
  ...
  while (i < str.length()) {
    if (!had_non_sign && str[i] == MINUS_CH) {
      ...
      // sexagesimal colon; dots are not allowed in first part of a sexagesimal
      // number
      i_colon++;
      had_non_sign = true;
      after_sign_e = false;
  ...
  // numbers in brackets is an internal reference to a stored MathStructure
  // object
  ...
  // handle - in front of the number (even number of minuses equals plus, odd
  // number equals a single minus)
  ...
  if (po.preserve_format) {
    // handle multiple - in front of the number (treated as a single sign if
    // po.preserve_format is false)
    while (minus_count > 0) {
      mstruct->transform(STRUCT_NEGATE);
      minus_count--;
    }
...
bool Calculator::parseOperators(MathStructure *mstruct, string str,
  ...
  // Parse \a (internal single character substitution for xor operators) as
  // bitwise xor
  ...
  // In adaptive parsing mode division might be handled differentiately
  // depending on usage of whitespace characters, e.g. 5/2 m = (5/2)*m,
  // 5/2m=5/(2m)
  ...
  // In conventional parsing mode there is not difference between implicit and
  ...
  // Parse \x1c (internal single substitution character for angle operator) for
  // complex angle format
  ...
  if ((i = str.find(ID_WRAP_RIGHT_CH, 1)) != string::npos &&
    ...
    if (b) {
      ...
      if (po.parsing_mode == PARSING_MODE_ADAPTIVE &&
        ...
        Prefix *p1 = NULL;
        bool b_plus = false;
        // In adaptive parsing mode, parse 5m 2cm as 5m+2cm, 5ft 2in as 5ft+2in,
        // and similar
