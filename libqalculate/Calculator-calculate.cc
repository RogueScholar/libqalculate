...
string Calculator::calculateAndPrint(string str, int msecs,
  ...
  if (separateToExpression(from_str, to_str, evalops, true)) {
    ...
    if (equalsIgnoreCase(to_str, "hex") ||
      ...
      // ? in front of unit epxression is interpreted as a request for the
      // optimal prefix.
      evalops.parse_options.units_enabled = true;
  ...
  if (!to_str.empty() && str != from_str) {
    mstruct =
        calculate(str, evalops, parsed_expression ? &parsed_struct : NULL);
  } else {
    // handle case where conversion to units requested, but original expression
    // and result does not contains any unit
    MathStructure to_struct;
    mstruct = calculate(str, evalops, &parsed_struct, &to_struct);
    if (to_struct.containsType(STRUCT_UNIT, true) &&
        !mstruct.contains(STRUCT_UNIT) &&
        !parsed_struct.containsType(STRUCT_UNIT, false, true, true)) {
      ...
      // recalculate
      if (!to_struct.isZero())
        mstruct = calculate(mstruct, evalops, to_str);
    }
  }
...
#define PRINT_CALENDAR(x, c)                                                   \
  ...
  if (parsed_expression) {
    ...
    if (po_parsed.base == BASE_CUSTOM &&
        ...
        customInputBase() >= -1114112L && customInputBase() <= 1114112L) {
  ...
  if (po.is_approximate && mstruct.isApproximate())
