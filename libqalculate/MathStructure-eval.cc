...
/*
   ...
   epxression separately for approriate subintervals.
...
bool calculate_nondifferentiable_functions(MathStructure &m,
                                           ...
                                           int i_type) {
  if (m.isFunction() && m.function() != eo.protected_function) {
    if ((i_type <= 0 && !function_differentiable(m.function())) ||
        ...
        if (mstruct[i][0].number().isPositive()) {
          if (i_ln == (size_t)-1) {
            ...
            if (b &&
              ...
              if (mstruct[i_ln][0].number().multiply(
          ...
          if (i_ln_m == (size_t)-1) {
            ...
            if (b && (b_m1 || mstruct[i][1][0].number().raise(
              ...
              if (mstruct[i_ln_m][1][0].number().multiply(
                      mstruct[i][1][0].number())) {
                mstruct.delChild(i + 1);
                b_ret = true;
                i--;
...
bool MathStructure::complexToPolarForm(const EvaluationOptions &eo) {
  if (m_type == STRUCT_NUMBER && o_number.hasImaginaryPart()) {
    ...
    case ANGLE_UNIT_DEGREES: {
      if (CALCULATOR->getDegUnit()) {
        marg.multiply(CALCULATOR->getDegUnit(), true);
      }
      break;
    }
    case ANGLE_UNIT_GRADIANS: {
      if (CALCULATOR->getGraUnit()) {
...
bool warn_ratio_units(MathStructure &m, bool top_level = true) {
  if (!top_level && m.isUnit() &&
    ...
    CALCULATOR->error(true,
                      "Logarithmic ratio units is treated as other units and "
...
MathStructure &MathStructure::eval(const EvaluationOptions &eo) {
      ...
      if (containsType(STRUCT_COMPARISON)) {
        ...
        test_munc:
          if (munc.isNumber()) {
            ...
            if (munc.isMultiplication()) {
              if (munc.size() == 2) {
                if (isMultiplication() && CHILD(0).isNumber() &&
                    (munc[1] == CHILD(1) ||
                     (munc[1].isFunction() &&
                      munc[1].function()->id() == FUNCTION_ID_ABS &&
                      munc[1].size() == 1 && CHILD(1) == munc[1][0]))) {
                  CHILD(0).number().setUncertainty(munc[0].number());
                  CHILD(0).numberUpdated();
                  CHILD_UPDATED(0)
                  b_failed = false;
                } else if (equals(munc[1]) ||
                           (munc[1].isFunction() &&
                            munc[1].function()->id() == FUNCTION_ID_ABS &&
                            munc[1].size() == 1 && equals(munc[1][0]))) {
                ...
                if (SIZE + 1 - i2 == munc.size()) {
                  ...
                  for (size_t i = 1; i < munc.size(); i++, i2++) {
                    if (!munc[i].equals(CHILD(i2)) &&
                        !(munc[i].isFunction() &&
                          munc[i].function()->id() == FUNCTION_ID_ABS &&
                          munc[i].size() == 1 && CHILD(i2) == munc[i][0])) {
                      b = false;
                      break;
                    }
                  }
          ...
          if (b_failed && munc.countTotalChildren(false) < 50) {
            if (one_prepended && munc.isMultiplication() && munc[0].isOne())
  ...
  if (eo.structuring != STRUCTURING_NONE) {
    ...
    if (eo.parse_options.angle_unit == ANGLE_UNIT_GRADIANS ||
