...
int MathStructure::merge_addition(MathStructure &mstruct,
        ...
        if (eo.transform_trigonometric_functions && CHILD(i2).isPower() &&
          ...
          if (eo.protected_function != CHILD(i2)[0].function()) {
            ...
            if (mstruct[0].isNumber()) {
              ...
              b = true;
              for (size_t i = 0; i < mstruct.size(); i++) {
                if (!mstruct[i].equals(CHILD(i2 - 1 >= i ? i + 1 : i + 2))) {
                  b = false;
                  break;
                }
              }
          ...
          if (CHILD(i2).function()->id() == FUNCTION_ID_SIGNUM &&
            ...
            if (equals(mstruct)) {
              ...
              case ANGLE_UNIT_DEGREES: {
                calculateMultiply(Number(90, 1, 0), eo);
                break;
              }
              case ANGLE_UNIT_GRADIANS: {
                calculateMultiply(Number(100, 1, 0), eo);
                break;
              }
          ...
          if (mstruct[i2].isPower() && mstruct[i2][0].isFunction() &&
            ...
            if (eo.protected_function != mstruct[i2][0].function()) {
              ...
              if (CHILD(0).isNumber()) {
                ...
                b = true;
                for (size_t i = 0; i < SIZE; i++) {
                  if (!CHILD(i).equals(mstruct[i2 - 1 >= i ? i + 1 : i + 2])) {
                    b = false;
                    break;
                  }
                }
  ...
  case STRUCT_POWER: {
    if (CHILD(0).isFunction() &&
        ...
        !FUNCTION_PROTECTED(eo, CHILD(0).function()->id() == FUNCTION_ID_SIN
                                    ? FUNCTION_ID_COS
                                    : FUNCTION_ID_SIN) &&
  ...
  case STRUCT_FUNCTION: {
    if (o_function->id() == FUNCTION_ID_SIGNUM && mstruct.isMultiplication() &&
        !FUNCTION_PROTECTED(eo, FUNCTION_ID_SIGNUM)) {
      if (SIZE == 2 && CHILD(0).isAddition() && CHILD(0).size() == 2 &&
        ...
        for (size_t im = 0; im < mstruct.size(); im++) {
          if (mstruct[im] == *this) {
            ...
            m2.delChild(im + 1, true);
            if ((CHILD(0)[0].isOne() && m2 == CHILD(0)[1]) ||
                (CHILD(0)[1].isOne() && m2 == CHILD(0)[0])) {
              setFunctionId(FUNCTION_ID_ABS);
              ERASE(1)
              MERGE_APPROX_AND_PREC(mstruct)
              return 1;
      ...
      case ANGLE_UNIT_RADIANS: {
        set(CALCULATOR->getVariableById(VARIABLE_ID_PI), true);
        multiply(nr_half);
...
int MathStructure::merge_multiplication(MathStructure &mstruct,
    ...
    case STRUCT_MULTIPLICATION: {
      ...
      if (reducables.size() > 0) {
        ...
        for (size_t i = 0; i < reducables.size(); i++) {
          ...
          case STRUCT_POWER: {
            if (CHILD(0)[reducables[i] - i_erased][1].isNumber() &&
                CHILD(0)[reducables[i] - i_erased][1].number().isReal()) {
              ...
              if (nrs[i] == CHILD(0)[reducables[i] - i_erased][1].number()) {
                ...
                if (CHILD(0)[reducables[i] - i_erased][1].number().isOne()) {
                  CHILD(0)
                  [reducables[i] - i_erased].setToChild(
                      1, true, &CHILD(0), reducables[i] - i_erased + 1);
                } else {
  ...
  if (isZero()) {
    if (mstruct.isFunction()) {
      if ((mstruct.function()->id() == FUNCTION_ID_LOG ||
           mstruct.function()->id() == FUNCTION_ID_EI) &&
          mstruct.size() == 1) {
      ...
      } else if (mstruct.function()->id() == FUNCTION_ID_LI &&
                 mstruct.size() == 1) {
      ...
      if ((mstruct[0].function()->id() == FUNCTION_ID_LOG ||
           mstruct[0].function()->id() == FUNCTION_ID_EI) &&
          mstruct[0].size() == 1) {
      ...
      } else if (mstruct[0].function()->id() == FUNCTION_ID_LI &&
                 mstruct[0].size() == 1) {
  ...
  case STRUCT_POWER: {
    ...
    case STRUCT_FUNCTION: {
      ...
      if (eo.transform_trigonometric_functions && CHILD(0).isFunction() &&
          CHILD(0).size() == 1 && mstruct.size() == 1 && CHILD(1).isNumber() &&
    ...
    default: {
      ...
      if (mstruct.isNumber() && CHILD(1).isNumber() &&
          !CHILD(1).number().includesInfinity() && CHILD(0).isNumber() &&
          CHILD(0).number().isRational() && !CHILD(0).number().isZero() &&
          mstruct.number().isRational()) {
        if (CHILD(0).isInteger() &&
            mstruct.number().denominator() == CHILD(0).number().numerator()) {
          CHILD(1).number()--;
          MERGE_APPROX_AND_PREC(mstruct)
          calculateRaiseExponent(eo);
          if (!mstruct.number().numeratorIsOne())
            calculateMultiply(mstruct.number().numerator(), eo, mparent,
                              index_this);
          return 1;
        } else if (mstruct.number().denominator() ==
                       CHILD(0).number().numerator() &&
                   mstruct.number().numerator() ==
                       CHILD(0).number().denominator()) {
          CHILD(1).number()--;
          MERGE_APPROX_AND_PREC(mstruct)
          calculateRaiseExponent(eo);
          return 1;
      ...
      if (CHILD(0).isFunction() && mstruct.isZero() &&
        ...
        if ((CHILD(0).function()->id() == FUNCTION_ID_LOG ||
             CHILD(0).function()->id() == FUNCTION_ID_EI) &&
            SIZE == 1) {
  ...
  case STRUCT_FUNCTION: {
    if (eo.protected_function != o_function) {
      ...
      } else if (o_function->id() == FUNCTION_ID_LI && SIZE == 1 &&
                 mstruct.isZero()) {
...
int MathStructure::merge_power(MathStructure &mstruct,
  ...
  if (m_type == STRUCT_NUMBER && o_number.isInfinite(false)) {
    if (mstruct.representsNegative(false)) {
      ...
      MERGE_APPROX_AND_PREC(mstruct)
      return 1;
    } else if (mstruct.representsNonZero(false) &&
               mstruct.representsPositive(false)) {
  ...
  case STRUCT_FUNCTION: {
    if (eo.protected_function != o_function) {
      ...
      } else if (o_function->id() == FUNCTION_ID_SIGNUM &&
                 CHILD(0).representsReal(true) && SIZE == 2 &&
      ...
      if (b) {
        ...
        for (size_t i = 0; i < mstruct.size(); i++) {
          ...
          if (!mstruct[i].representsReal(true) ||
              (isZero() && !mstruct[i].representsPositive(true)))
            continue;
          if (i > 0)
            mthis[1] = mstruct[i];
          EvaluationOptions eo2 = eo;
          ...
          if (mthis.calculateRaiseExponent(eo2) &&
              (!mthis.isPower() ||
               ((!isFunction() || o_function->id() != FUNCTION_ID_ABS ||
                 SIZE != 1 || !CHILD(0).equals(mthis[0], true, true)) &&
    ...
    if (CHILD(0) == mstruct[0]) {
      ...
      default: {
        ...
        return -1;
               ...
               CHILD(1).isNumber() && mstruct.contains(CHILD(0))) {
      mstruct.calculateReplace(CHILD(0), CHILD(1), eo);
      if (eo.isolate_x) {
        mstruct.isolate_x(eo, eo);
        mstruct.calculatesub(eo, eo, true);
      ...
      return 1;
               ...
               mstruct[1].isNumber() && contains(mstruct[0])) {
      calculateReplace(mstruct[0], mstruct[1], eo);
      if (eo.isolate_x) {
        isolate_x(eo, eo);
        calculatesub(eo, eo, true);
...
bool MathStructure::calculatesub(const EvaluationOptions &eo,
  ...
  case STRUCT_LOGICAL_OR: {
    ...
    switch (CHILD(0).type()) {
    case STRUCT_MULTIPLICATION: {
      if (CHILD(0)[1] != 0 && CHILD(0)[1].unit() &&
          CHILD(0)[1].unit()->name().find("ohm") != string::npos) {
        isResistance = true;
      }
      break;
    }
    case STRUCT_UNIT: {
      if (CHILD(0).unit() &&
          CHILD(0).unit()->name().find("ohm") != string::npos) {
        isResistance = true;
      }
      break;
    }
    ...
    for (size_t i = 0; i < SIZE; i++) {
      ...
      if (arg) {
        ...
        if (arg->handlesVector()) {
          if (arg->tests() && !CHILD(i).isVector() &&
              !CHILD(i).representsScalar()) {
            CHILD(i).eval(eo);
            CHILD_UPDATED(i);
          }
          if (CHILD(i).isVector()) {
