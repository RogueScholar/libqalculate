...
const string &PrintOptions::comma() const {
    if(comma_sign.empty()) return CALCULATOR->getComma();
    return comma_sign;
}
const string &PrintOptions::decimalpoint() const {
    if(decimalpoint_sign.empty()) return CALCULATOR->getDecimalPoint();
...
void Calculator::expressionItemDeleted(ExpressionItem *item) {
    ...
    for(size_t i2 = 1; i2 <= item->countNames(); i2++) {
        if(item->type() == TYPE_VARIABLE || item->type() == TYPE_UNIT) {
            for(size_t i = 0; i < variables.size(); i++) {
                if(!variables[i]->isLocal() && !variables[i]->isActive() && variables[i]->hasName(item->getName(i2).name, item->getName(i2).case_sensitive) && !getActiveExpressionItem(variables[i])) {
...
bool Calculator::parseOperators(MathStructure *mstruct, string str, const ParseOptions &po) {
    ...
    if(!po.rpn) {
        while(po.base != BASE_ROMAN_NUMERALS && (i = str.find('|', i)) != string::npos) {
            if(i == 0 || i == str.length() - 1 || is_in(OPERATORS INTERNAL_OPERATORS SPACE, str[i - 1])) {
                b_abs_or = true;
                break;
            }
            if(str[i + 1] == '|') {
    ...
    if((i = str.find(ID_WRAP_RIGHT_CH, 1)) != string::npos && i + 1 != str.length()) {
        ...
        if(b) {
            ...
            if(po.parsing_mode == PARSING_MODE_ADAPTIVE && mstruct->isMultiplication() && mstruct->size() >= 2 && !(*mstruct)[0].inParentheses()) {
                ...
                Prefix *p1 = NULL;
                bool b_plus = false;
                if((*mstruct)[0].isMultiplication() && (*mstruct)[0].size() == 2 && (*mstruct)[0][0].isNumber() && (*mstruct)[0][1].isUnit()) {
                    u1 = (*mstruct)[0][1].unit();
                            ...
                            if(str_names != &default_prop_names && !default_prop_names.empty()) {
                                ...
                                while(true) {
                                    ...
                                    if(b_prop_ref || (!had_ref && names_i == string::npos)) {
                                        had_ref = true;
                                        if(names_i == string::npos) {
                                            stmp = default_prop_names.substr(i3, default_prop_names.length() - i3);
                                        }
                                        else {
                    ...
                    for(size_t i2 = 1; i2 <= functions[i]->lastArgumentDefinitionIndex(); i2++) {
                        ...
                        if(arg) {
                            ...
                            case ARGUMENT_TYPE_DATA_OBJECT: {
                                xmlNewProp(newnode, (xmlChar*) "type", (xmlChar*) "data-object");
                                break;
                            }
                            case ARGUMENT_TYPE_DATA_PROPERTY: {
                                xmlNewProp(newnode, (xmlChar*) "type", (xmlChar*) "data-property");
                                break;
                            }
                            default: {
...
bool Calculator::fetchExchangeRates(int timeout, int n) {
    ...
    if(sbuffer.empty()) {
        error(true, _("Failed to download exchange rates from %s: %s."), "ECB", "Document empty", NULL);
        FETCH_FAIL_CLEANUP;
        return false;
    }
    ...
    if(n >= 2) {
        ...
        if(res != CURLE_OK) {
            error(true, _("Failed to download exchange rates from %s: %s."), "coinbase.com", error_buffer, NULL);
            FETCH_FAIL_CLEANUP;
            return false;
        }
        if(sbuffer.empty()) {
            error(true, _("Failed to download exchange rates from %s: %s."), "coinbase.com", "Document empty", NULL);
            FETCH_FAIL_CLEANUP;
            return false;
        }
    ...
    if(n >= 3) {
        ...
        if(res != CURLE_OK) {
            error(true, _("Failed to download exchange rates from %s: %s."), "mycurrency.net", error_buffer, NULL);
            FETCH_FAIL_CLEANUP;
            return false;
        }
        if(sbuffer.empty() || sbuffer.find("Internal Server Error") != string::npos) {
            error(true, _("Failed to download exchange rates from %s: %s."), "mycurrency.net", "Document empty", NULL);
            FETCH_FAIL_CLEANUP;
            return false;
        }
