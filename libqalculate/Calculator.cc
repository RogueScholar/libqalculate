...
PrintOptions::PrintOptions()
    : min_exp(EXP_PRECISION), base(BASE_DECIMAL), lower_case_numbers(false),
...
void CalculateThread::run() {
  ...
  while (true) {
    ...
    if (b_parse) {
      ...
      mstruct->setAborted();
      mstruct->set(
          CALCULATOR->calculate(meval, CALCULATOR->tmp_evaluationoptions));
    }
...
Calculator::Calculator() {
  ...
  if ((local_digit_group_separator.length() == 1 &&
       local_digit_group_separator[0] < 0) ||
  ...
  ILLEGAL_IN_NAMES = "\a\b" + DOT_S +
                     RESERVED OPERATORS SPACES PARENTHESISS VECTOR_WRAPS COMMAS;
  ILLEGAL_IN_NAMES_MINUS_SPACE_STR =
      "\a\b" + DOT_S + RESERVED OPERATORS PARENTHESISS VECTOR_WRAPS COMMAS;
...
bool Calculator::utf8_pos_is_valid_in_name(char *pos) {
  ...
  if ((unsigned char)pos[0] >= 0xC0) {
    ...
    return str != SIGN_DIVISION && str != SIGN_DIVISION_SLASH &&
           str != SIGN_MULTIPLICATION && str != SIGN_MULTIDOT &&
           str != SIGN_SMALLCIRCLE && str != SIGN_MULTIBULLET &&
           str != SIGN_MINUS && str != SIGN_PLUS && str != SIGN_NOT_EQUAL &&
           str != SIGN_GREATER_OR_EQUAL && str != SIGN_LESS_OR_EQUAL;
  }
  return true;
}
...
ExpressionItem *Calculator::getActiveExpressionItem(ExpressionItem *item) {
  ...
  for (size_t i = 1; i <= item->countNames(); i++) {
    ExpressionItem *item2 =
        getActiveExpressionItem(item->getName(i).name, item);
    if (item2) {
      return item2;
    }
  }
...
ExpressionItem *Calculator::getActiveExpressionItem(string name,
                                                    ExpressionItem *item) {
  if (name.empty())
    return NULL;
...
Prefix *Calculator::getPrefix(string name_) const {
  for (size_t i = 0; i < prefixes.size(); i++) {
    if (prefixes[i]->shortName(false) == name_ ||
        prefixes[i]->longName(false) == name_ ||
        prefixes[i]->unicodeName(false) == name_) {
      return prefixes[i];
    }
  }
  return NULL;
...
size_t Calculator::parseAddId(MathFunction *f, const string &str,
                              const ParseOptions &po, bool persistent) {
  size_t id = 0;
  if (priv->freed_ids.size() > 0) {
    id = priv->freed_ids.back();
    priv->freed_ids.pop_back();
...
size_t Calculator::parseAddIdAppend(MathFunction *f,
                                    const MathStructure &append_mstruct,
                                    const string &str, const ParseOptions &po,
                                    bool persistent) {
  size_t id = 0;
  if (priv->freed_ids.size() > 0) {
    id = priv->freed_ids.back();
...
size_t Calculator::parseAddVectorId(const string &str, const ParseOptions &po,
                                    bool persistent) {
  size_t id = 0;
  if (priv->freed_ids.size() > 0) {
    id = priv->freed_ids.back();
...
void Calculator::addBuiltinVariables() {
  ...
  v_x = (UnknownVariable *)addVariable(
      new UnknownVariable("", "x", "", true, false));
  v_y = (UnknownVariable *)addVariable(
      new UnknownVariable("", "y", "", true, false));
  v_z = (UnknownVariable *)addVariable(
      new UnknownVariable("", "z", "", true, false));
...
void Calculator::addBuiltinUnits() {
  ...
  u_btc = addUnit(new AliasUnit(_("Currency"), "BTC", "bitcoins", "bitcoin",
                                "Bitcoins", u_euro, "7377.47", 1, "", false,
                                true, true));
...
void Calculator::clearBuffers() {
  for (unordered_map<size_t, bool>::iterator it = priv->ids_p.begin();
       it != priv->ids_p.end(); ++it) {
    if (!it->second) {
...
string Calculator::localizeExpression(string str,
  ...
  if ((DOT_STR == DOT && COMMA_STR == COMMA && !po.comma_as_separator) ||
...
string Calculator::unlocalizeExpression(string str,
  ...
  if (DOT_STR != DOT) {
    if (DOT_STR == COMMA && str.find(COMMA_STR) == string::npos && base > 0 &&
        base <= 10) {
      bool b_vector =
          (str.find(LEFT_VECTOR_WRAP) != string::npos && !po.dot_as_separator);
      ...
      while (ui != string::npos) {
        for (; ui2 < q_end.size(); ui2++) {
          if (ui >= q_begin[ui2]) {
            if (ui <= q_end[ui2]) {
              ui = str.find_first_of(b_vector ? DOT COMMA : COMMA,
                                     q_end[ui2] + 1);
              if (ui == string::npos)
                break;
            }
        ...
        if (ui > 0) {
          ...
          if (ui3 != string::npos &&
              ((str[ui3] > 'a' && str[ui3] < 'z') ||
               (str[ui3] > 'A' && str[ui3] < 'Z')) &&
              is_not_number(str[ui3], base))
            return str;
        }
        if (ui != str.length() - 1) {
          size_t ui3 = str.find_first_not_of(SPACES, ui + 1);
          if (ui3 != string::npos && is_not_number(str[ui3], base))
            return str;
          if (b_vector || !b_dot) {
            ui3 = str.find_first_not_of(SPACES NUMBERS, ui3 + 1);
            if (ui3 != string::npos &&
                (str[ui3] == COMMA_CH || (b_vector && str[ui3] == DOT_CH)))
              return str;
          }
        }
...
string Calculator::calculateAndPrint(string str, int msecs,
  ...
  bool do_bases = false, do_factors = false, do_fraction = false,
       do_pfe = false, do_calendars = false, do_expand = false,
       do_binary_prefixes = false, complex_angle_form = false;
  ...
  if (separateToExpression(from_str, to_str, evalops, true)) {
    ...
    if (equalsIgnoreCase(to_str, "hex") ||
        EQUALS_IGNORECASE_AND_LOCAL(to_str, "hexadecimal", _("hexadecimal"))) {
      str = from_str;
      ...
      printops.base = BASE_BIJECTIVE_26;
               ...
               equalsIgnoreCase(to_str, "sexagesimal") ||
               equalsIgnoreCase(to_str, _("sexagesimal"))) {
      str = from_str;
      ...
      if (to_str2 == "b26" || to_str2 == "B26")
        ...
        printops.base = BASE_UNICODE;
               ...
               equalsIgnoreCase(to_str2, "supergolden ratio") || to_str2 == "ψ")
...
#define PRINT_CALENDAR(x, c)                                                   \
  ...
  if (b_fail) {                                                                \
    ...
    if (!b_fail) {
      str += " (";
      str += chineseStemName(st);
      str += string(" ");
      str += chineseBranchName(br);
      str += ")";
    }
...
extern bool replace_function(MathStructure &m, MathFunction *f1,
                             MathFunction *f2, const EvaluationOptions &eo);
extern bool replace_intervals_f(MathStructure &mstruct);
extern bool replace_f_interval(MathStructure &mstruct,
                               const EvaluationOptions &eo);
    ...
    return true;
             ...
             (mstruct.variable()->referenceName() == "ans" ||
              (mstruct.variable()->referenceName().length() == 4 &&
               ...
               is_in(NUMBERS, mstruct.variable()->referenceName()[4])))) {
  ...
  if (m.isComparison()) {
    if (m.comparisonType() == COMPARISON_EQUALS) {
      ...
      if (m[0].isFunction() && m[1].isFunction() &&
        ...
        if (m[1].containsInterval(true) ||
            m[1].containsFunction(CALCULATOR->f_interval, true)) {
          MathStructure mv(m[1]);
          replace_f_interval(mv, eo);
          replace_intervals_f(mv);
          if (!mstruct.replace(m[0], mv))
            CALCULATOR->error(false, _("Original value (%s) was not found."),
                              format_and_print(m[0]).c_str(), NULL);
        } else {
          if (!mstruct.replace(m[0], m[1]))
            CALCULATOR->error(false, _("Original value (%s) was not found."),
                              format_and_print(m[0]).c_str(), NULL);
        }
      }
...
MathStructure Calculator::calculate(string str, const EvaluationOptions &eo,
  ...
  if (!str_where.empty()) {
    ...
    if (mstruct.isComparison() ||
        (mstruct.isFunction() && mstruct.function() == CALCULATOR->f_solve &&
         mstruct.size() >= 1 && mstruct[0].isComparison())) {
      beginTemporaryStopMessages();
...
MathStructure Calculator::convertToMixedUnits(const MathStructure &mstruct,
  ...
  if (mstruct[1].isUnit() &&
    ...
    Number nr_one(1, 1);
    Number nr_ten(10, 1);
    while (eo.mixed_units_conversion > MIXED_UNITS_CONVERSION_DOWNWARDS &&
           nr.isGreaterThan(nr_one)) {
      Unit *best_u = NULL;
      Number best_nr;
      int best_priority = 0;
      for (size_t i = 0; i < units.size(); i++) {
    ...
    while (
        ...
        (((AliasUnit *)u)->mixWithBase() != 0 ||
         eo.mixed_units_conversion == MIXED_UNITS_CONVERSION_FORCE_ALL ||
         (eo.mixed_units_conversion == MIXED_UNITS_CONVERSION_FORCE_INTEGER &&
          ((AliasUnit *)u)->expression().find_first_not_of(NUMBERS) ==
              string::npos)) &&
        !nr.isInteger() && !nr.isZero()) {
      Number int_nr(nr);
      int_nr.trunc();
      if (eo.mixed_units_conversion == MIXED_UNITS_CONVERSION_DOWNWARDS_KEEP &&
          int_nr.isZero())
        break;
      nr -= int_nr;
      MathStructure mstruct_nr(nr);
      ...
      while (
          !accept_obsolete &&
          ((AliasUnit *)u)->firstBaseUnit()->subtype() == SUBTYPE_ALIAS_UNIT &&
          abs(((AliasUnit *)((AliasUnit *)u)->firstBaseUnit())->mixWithBase()) >
              1) {
      ...
      if (!mstruct_nr.isNumber() || !m_exp.isOne())
        break;
      if (eo.mixed_units_conversion == MIXED_UNITS_CONVERSION_FORCE_ALL &&
          mstruct_nr.number().isLessThanOrEqualTo(nr))
        break;
      u = ((AliasUnit *)u)->firstBaseUnit();
...
MathStructure Calculator::convert(const MathStructure &mstruct, Unit *to_unit,
  ...
  if (to_unit->hasNonlinearRelationTo(to_unit->baseUnit()) &&
    ...
    mstruct_new = convert(mstruct, to_unit->baseUnit(), eo, always_convert,
                          convert_to_mixed_units);
  ...
  if (mstruct_new.isAddition()) {
    if (mstruct_new.size() > 100 && aborted())
      return mstruct;
  ...
  if (!mstruct_new.isPower() && !mstruct_new.isUnit() &&
      !mstruct_new.isMultiplication()) {
      ...
      if (cu2) {
        ...
        case STRUCT_MULTIPLICATION: {
          for (size_t i = 1; i <= mstruct_new.countChildren(); i++) {
            if (mstruct_new.countChildren() > 100 && aborted())
              return mstruct;
    ...
    if (b) {
      ...
      if (mstruct_new.containsType(STRUCT_UNIT)) {
        ...
        mbak.eval(eo2);
        if (!mbak.containsType(STRUCT_UNIT))
          mstruct_new = mbak;
      }

      if (cu) {
        MathStructure mstruct_cu(
        ...
        mstruct_new.multiply(MathStructure(
...
MathStructure Calculator::convertToOptimalUnit(const MathStructure &mstruct,
  ...
  case STRUCT_MULTIPLICATION: {
    ...
    for (size_t i = 1; i <= mstruct_old.countChildren(); i++) {
      if (mstruct_old.countChildren() > 100 && aborted())
        return mstruct_old;
      if (mstruct_old.getChild(i)->isUnit()) {
        ...
        mstruct_old[i - 1] =
            convertToOptimalUnit(mstruct_old[i - 1], eo, convert_to_si_units);
        mstruct_old.childUpdated(i);
        if (!mstruct_old[i - 1].equals(mstruct[i - 1], true, true))
          child_updated = true;
      }
    }
    if (child_updated)
      mstruct_old.eval(eo2);
        ...
        if (mstruct_new.getChild(i)->isUnit()) {
          ...
          cu->add(mstruct_new.getChild(i)->base()->unit(),
                  mstruct_new.getChild(i)->exponent()->number().intValue());
                   ...
                   !mstruct_new.getChild(i)->containsInterval(true, true, false,
                                                              1, true) &&
                   !aborted()) {
          MathStructure m_i_old(mstruct_new[i - 1]);
          mstruct_new[i - 1] =
              convertToOptimalUnit(mstruct_new[i - 1], eo, convert_to_si_units);
          mstruct_new.childUpdated(i);
          if (!mstruct_new[i - 1].equals(m_i_old, true, true))
            child_updated = true;
        }
      }
    ...
    if ((eo.approximation == APPROXIMATION_EXACT &&
      ...
      return mstruct_old;
    if (mstruct_new.equals(mstruct_old, true, true))
      return mstruct_old;
    int new_points = 0;
    bool new_minus = true;
    bool new_is_si_units = true;
...
MathStructure Calculator::convert(const MathStructure &mstruct_to_convert,
  ...
  if (u) {
    ...
    current_stage = MESSAGE_STAGE_CONVERSION;
    if (to_struct)
      to_struct->set(cu.generateMathStructure());
    if (cu.countUnits() > 0) {
...
Unit *Calculator::getActiveUnit(string name_) {
  if (name_.empty())
    return NULL;
  for (size_t i = 0; i < units.size(); i++) {
    if (units[i]->isActive() && units[i]->subtype() != SUBTYPE_COMPOSITE_UNIT &&
        units[i]->hasName(name_)) {
      return units[i];
    }
  }
        ...
        if (it == ufvl.end()) {
          ...
          if (l < l2 ||
              ...
              (item->type() == TYPE_FUNCTION && l == l2 &&
               (ufvl_t[i] != 'p' && ufvl_t[i] != 'P' && ufvl_t[i] != 'q')) ||
...
bool compare_name(const string &name, const string &str,
                  const size_t &name_length, const size_t &str_index,
                  int base) {
  if (name_length == 0)
    return false;
  ...
  if (name_length == 1) {
    if (base < 2 || base > 10)
      return is_not_number(str[str_index], base);
    return true;
  }
  for (size_t i = 1; i < name_length; i++) {
    if (name[i] != str[str_index + i])
      return false;
  }
  if (base < 2 || base > 10) {
    for (size_t i = 0; i < name_length; i++) {
      if (is_not_number(str[str_index + i], base))
        return true;
    }
    return false;
  }
...
size_t compare_name_no_case(const string &name, const string &str,
                            const size_t &name_length, const size_t &str_index,
                            int base) {
  if (name_length == 0)
    return 0;
...
void Calculator::parseSigns(string &str,
                            bool convert_to_internal_representation) const {
  vector<size_t> q_begin;
  vector<size_t> q_end;
  ...
  while (true) {
    ...
    if (ui != string::npos &&
        (ui == str.length() - 2 ||
         (str[ui + 2] != -80 && (str[ui + 2] < -76 || str[ui + 2] > -71))))
      ui = string::npos;
    size_t ui2 = str.find('\xc2', prev_ui == string::npos ? 0 : prev_ui);
    if (ui2 != string::npos &&
        (ui2 == str.length() - 1 ||
         (str[ui2 + 1] != -71 && str[ui2 + 1] != -77 && str[ui2 + 1] != -78)))
      ui2 = string::npos;
    if (ui2 != string::npos && (ui == string::npos || ui2 < ui))
      ui = ui2;
    if (ui != string::npos) {
      for (size_t ui3 = 0; ui3 < q_end.size(); ui3++) {
        if (ui <= q_end[ui3] && ui >= q_begin[ui3]) {
          ...
          if (ui != string::npos &&
              (ui == str.length() - 2 ||
               (str[ui + 2] != -80 &&
                (str[ui + 2] < -76 || str[ui + 2] > -71))))
            ui = string::npos;
          ui2 = str.find('\xc2', q_end[ui3] + 1);
          if (ui2 != string::npos &&
              (ui2 == str.length() - 1 ||
               (str[ui2 + 1] != -71 && str[ui2 + 1] != -77 &&
                str[ui2 + 1] != -78)))
            ui2 = string::npos;
          if (ui2 != string::npos && (ui == string::npos || ui2 < ui))
            ui = ui2;
    ...
    if (str[ui] == '\xc2') {
      if (str[ui + 1] == -71)
        str.replace(ui, 2, ui == prev_ui ? "1)" : "^(1)");
      else if (str[ui + 1] == -78)
        str.replace(ui, 2, ui == prev_ui ? "2)" : "^(2)");
      else if (str[ui + 1] == -77)
        str.replace(ui, 2, ui == prev_ui ? "3)" : "^(3)");
    } else {
      if (str[ui + 2] == -80)
        str.replace(ui, 3, ui == prev_ui ? "0)" : "^(0)");
      else if (str[ui + 2] == -76)
        str.replace(ui, 3, ui == prev_ui ? "4)" : "^(4)");
      else if (str[ui + 2] == -75)
        str.replace(ui, 3, ui == prev_ui ? "5)" : "^(5)");
      else if (str[ui + 2] == -74)
        str.replace(ui, 3, ui == prev_ui ? "6)" : "^(6)");
      else if (str[ui + 2] == -73)
        str.replace(ui, 3, ui == prev_ui ? "7)" : "^(7)");
      else if (str[ui + 2] == -72)
        str.replace(ui, 3, ui == prev_ui ? "8)" : "^(8)");
      else if (str[ui + 2] == -71)
        str.replace(ui, 3, ui == prev_ui ? "9)" : "^(9)");
    }
    if (ui == prev_ui) {
  ...
  while (true) {
    ...
    if (ui != string::npos &&
        (ui == str.length() - 2 || str[ui + 2] < -112 || str[ui + 2] > -98))
      ui = string::npos;
    if (ui != string::npos) {
      for (size_t ui3 = 0; ui3 < q_end.size(); ui3++) {
        if (ui <= q_end[ui3] && ui >= q_begin[ui3]) {
          ui = str.find("\xe2\x85", q_end[ui3] + 1);
          if (ui != string::npos && (ui == str.length() - 2 ||
                                     str[ui + 2] < -112 || str[ui + 2] > -98))
            ui = string::npos;
          if (ui == string::npos)
            break;
        }
    ...
    if (ui == string::npos)
      break;
    space_n = 0;
    while (ui > 0 && ui - 1 - space_n != 0 && str[ui - 1 - space_n] == SPACE_CH)
      space_n++;
    ...
    if (str[ui + 2] == -98)
      str.replace(ui, 3, b_add ? "+(7/8)" : "(7/8)");
    else if (str[ui + 2] == -99)
      str.replace(ui, 3, b_add ? "+(5/8)" : "(5/8)");
    else if (str[ui + 2] == -100)
      str.replace(ui, 3, b_add ? "+(3/8)" : "(3/8)");
    else if (str[ui + 2] == -101)
      str.replace(ui, 3, b_add ? "+(1/8)" : "(1/8)");
    else if (str[ui + 2] == -102)
      str.replace(ui, 3, b_add ? "+(5/6)" : "(5/6)");
    else if (str[ui + 2] == -103)
      str.replace(ui, 3, b_add ? "+(1/6)" : "(1/6)");
    else if (str[ui + 2] == -104)
      str.replace(ui, 3, b_add ? "+(4/5)" : "(4/5)");
    else if (str[ui + 2] == -105)
      str.replace(ui, 3, b_add ? "+(3/5)" : "(3/5)");
    else if (str[ui + 2] == -106)
      str.replace(ui, 3, b_add ? "+(2/5)" : "(2/5)");
    else if (str[ui + 2] == -107)
      str.replace(ui, 3, b_add ? "+(1/5)" : "(1/5)");
    else if (str[ui + 2] == -108)
      str.replace(ui, 3, b_add ? "+(2/3)" : "(2/3)");
    else if (str[ui + 2] == -109)
      str.replace(ui, 3, b_add ? "+(1/3)" : "(1/3)");
    else if (str[ui + 2] == -110) {
      str.replace(ui, 3, b_add ? "+(1/10)" : "(1/10)");
      ui++;
    } else if (str[ui + 2] == -111)
      str.replace(ui, 3, b_add ? "+(1/9)" : "(1/9)");
    else if (str[ui + 2] == -112)
      str.replace(ui, 3, b_add ? "+(1/7)" : "(1/7)");
    if (b_add)
      prev_ui = ui + 6;
    else
      prev_ui = ui + 5;
  ...
  while (true) {
    ...
    if (ui != string::npos &&
        (ui == str.length() - 1 ||
         (str[ui + 1] != -66 && str[ui + 1] != -67 && str[ui + 1] != -68)))
      ui = string::npos;
    if (ui != string::npos) {
      for (size_t ui3 = 0; ui3 < q_end.size(); ui3++) {
        if (ui <= q_end[ui3] && ui >= q_begin[ui3]) {
    ...
    if (str[ui + 1] == -66)
      str.replace(ui, 2, b_add ? "+(3/4)" : "(3/4)");
    else if (str[ui + 1] == -67)
      str.replace(ui, 2, b_add ? "+(1/2)" : "(1/2)");
    else if (str[ui + 1] == -68)
      str.replace(ui, 2, b_add ? "+(1/4)" : "(1/4)");
    if (b_add)
      prev_ui = ui + 6;
    else
      prev_ui = ui + 5;
    ...
    for (size_t i = 0; i < INTERNAL_SIGNS_COUNT; i += 2) {
      ...
      while (ui != string::npos) {
        ...
        if (ui == string::npos)
          break;
        int index_shift =
            strlen(internal_signs[i + 1]) - strlen(internal_signs[i]);
        for (size_t ui3 = ui2; ui3 < q_begin.size(); ui3++) {
          q_begin[ui3] += index_shift;
          q_end[ui3] += index_shift;
        }
    ...
    if (i_quote == string::npos || i_dquote < i_quote) {
      ...
      if (!b) {
        ...
        if (i_prev != string::npos) {
          ...
          if (i_op == string::npos)
            str +=
                b_degree ? "arcsec" RIGHT_PARENTHESIS : "in" RIGHT_PARENTHESIS;
          else
          ...
          set_function:
            ...
            if (f->args() == 0) {
              ...
              if (b && i5 >= 2) {
                ...
                if (f == f_vector) {
                  stmp += i2s(parseAddVectorId(stmp2, po));
                } else if ((f == f_interval || f == f_uncertainty) &&
...
#define BASE_2_10                                                              \
  ((po.base >= 2 && po.base <= 10) ||                                          \
   (po.base < BASE_CUSTOM && po.base != BASE_UNICODE &&                        \
    po.base != BASE_BIJECTIVE_26) ||                                           \
   (po.base == BASE_CUSTOM && priv->custom_input_base_i <= 10))

bool Calculator::parseNumber(MathStructure *mstruct, string str,
                             const ParseOptions &po) {
  mstruct->clear();
  if (str.empty())
    return false;
  if (str.find_first_not_of(OPERATORS "\a%\x1c" SPACE) == string::npos &&
      (po.base != BASE_ROMAN_NUMERALS || str.find("|") == string::npos)) {
    gsub("\a",
         str.find_first_of("%" OPERATORS) != string::npos ? " xor " : "xor",
         str);
    gsub("\x1c", "∠", str);
  ...
  if (str[0] == ID_WRAP_LEFT_CH && str.length() > 2 &&
      str[str.length() - 1] == ID_WRAP_RIGHT_CH) {
  ...
  if ((BASE_2_10 || po.base == BASE_DUODECIMAL) &&
      (itmp = str.find_first_not_of(
           po.base == BASE_DUODECIMAL
               ? NUMBER_ELEMENTS INTERNAL_NUMBER_CHARS MINUS DUODECIMAL_CHARS
               : NUMBER_ELEMENTS INTERNAL_NUMBER_CHARS EXPS MINUS,
           0)) != string::npos) {
    if (itmp == 0) {
      error(true, _("\"%s\" is not a valid variable/function/unit."),
            str.c_str(), NULL);
      if (minus_count % 2 == 1 && !po.preserve_format) {
        mstruct->set(-1, 1, 0);
      } else if (has_sign) {
      ...
      error(true,
            _("Trailing characters \"%s\" (not a valid variable/function/unit) "
              "in number \"%s\" was ignored."),
            stmp.c_str(), str.c_str(), NULL);
...
bool Calculator::parseAdd(string &str, MathStructure *mstruct,
                          const ParseOptions &po) {
  if (str.length() > 0) {
    size_t i;
    if (BASE_2_10) {
      i = str.find_first_of(SPACE MULTIPLICATION_2 OPERATORS INTERNAL_OPERATORS
                                PARENTHESISS EXPS ID_WRAP_LEFT,
                            1);
    } else {
      i = str.find_first_of(SPACE MULTIPLICATION_2 OPERATORS INTERNAL_OPERATORS
                                PARENTHESISS ID_WRAP_LEFT,
                            1);
    }
    if (i == string::npos && str[0] != LOGICAL_NOT_CH &&
        str[0] != BITWISE_NOT_CH &&
        !(str[0] == ID_WRAP_LEFT_CH &&
          str.find(ID_WRAP_RIGHT) < str.length() - 1)) {
...
bool Calculator::parseAdd(string &str, MathStructure *mstruct,
                          const ParseOptions &po, MathOperation s,
                          bool append) {
  if (str.length() > 0) {
    size_t i;
    if (BASE_2_10) {
    ...
    if (i == string::npos && str[0] != LOGICAL_NOT_CH &&
      ...
      if (s == OPERATION_EXP10 && po.read_precision == ALWAYS_READ_PRECISION) {
        ...
        if (s == OPERATION_EXP10 && !po.preserve_format &&
            mstruct->isNumber() && mstruct2->isNumber()) {
...
MathStructure *get_out_of_negate(MathStructure &mstruct, int *i_neg) {
  if (mstruct.isNegate() || (mstruct.isMultiplication() &&
                             mstruct.size() == 2 && mstruct[0].isMinusOne())) {
    (*i_neg)++;
...
bool Calculator::parseOperators(MathStructure *mstruct, string str,
                                const ParseOptions &po) {
  string save_str = str;
  mstruct->clear();
  ...
  string str2, str3;
  bool extended_roman =
      (po.base == BASE_ROMAN_NUMERALS && (i = str.find("|")) != string::npos &&
       i + 1 < str.length() && str[i + 1] == RIGHT_PARENTHESIS_CH);
  while (!extended_roman) {
    ...
    while (true) {
      ...
      if (i > 0 && i2 + 1 < str.length() && str[i - 1] == LEFT_PARENTHESIS_CH &&
          str[i2 + 1] == RIGHT_PARENTHESIS_CH) {
    ...
    if (i > 0 &&
        is_not_in(
            MULTIPLICATION_2 OPERATORS INTERNAL_OPERATORS PARENTHESISS SPACE,
            str[i - 1]) &&
        (!BASE_2_10 || (str[i - 1] != EXP_CH && str[i - 1] != EXP2_CH))) {
      if (po.rpn) {
    ...
    if (i2 + 1 < str.length() &&
        is_not_in(
            MULTIPLICATION_2 OPERATORS INTERNAL_OPERATORS PARENTHESISS SPACE,
            str[i2 + 1]) &&
        (!BASE_2_10 || (str[i2 + 1] != EXP_CH && str[i2 + 1] != EXP2_CH))) {
      if (po.rpn) {
    ...
    if (po.rpn && i > 0 && i2 + 1 == str.length() &&
        is_not_in(PARENTHESISS SPACE, str[i - 1])) {
      str += MULTIPLICATION_CH;
    }
    str2 = str.substr(i + 1, i2 - (i + 1));
    MathStructure *mstruct2 = new MathStructure();
    if (str2.empty()) {
      error(false, "Empty expression in parentheses interpreted as zero.",
            NULL);
    } else {
  ...
  if (!po.rpn) {
    while (po.base != BASE_ROMAN_NUMERALS &&
      ...
      if (i == 0 || i == str.length() - 1 ||
          is_in(OPERATORS INTERNAL_OPERATORS SPACE, str[i - 1])) {
        b_abs_or = true;
        break;
      }
      if (str[i + 1] == '|') {
    ...
    while ((i = str.find('|', 0)) != string::npos && i + 1 != str.length()) {
      if (str[i + 1] == '|') {
        ...
        while ((i2 = str.find("||", i2 + 2)) != string::npos) {
          if (is_in(OPERATORS INTERNAL_OPERATORS, str[i2 - 1]))
            depth++;
          else
            depth--;
          if (depth == 0)
        ...
        str3 += ID_WRAP_RIGHT;
        if (i2 == string::npos)
          str.replace(i, str.length() - i, str3);
        else
  ...
  if (po.rpn) {
    ...
    while (true) {
      ...
      if (i == string::npos) {
        ...
        if (!str2.empty()) {
          error(false,
                _("RPN syntax error. Values left at the end of the RPN "
                  "expression."),
                NULL);
          ...
          if (last_operator == 0 && mstack.size() > 1) {
            ...
            while (mstack.size() > 1) {
              ...
              case DIVISION_CH: {
                if (po.preserve_format) {
                  mstack[mstack.size() - 2]->transform_nocopy(STRUCT_DIVISION,
                                                              mstack.back());
                } else {
              ...
              case AND_CH: {
                mstack[mstack.size() - 2]->transform_nocopy(STRUCT_BITWISE_AND,
                                                            mstack.back());
                mstack.pop_back();
                break;
              }
              case OR_CH: {
                mstack[mstack.size() - 2]->transform_nocopy(STRUCT_BITWISE_OR,
                                                            mstack.back());
                mstack.pop_back();
                break;
              }
              case GREATER_CH: {
                if (last_operator2 == GREATER_CH) {
                  ...
                  mstack[mstack.size() - 2]->add_nocopy(
                      mstack.back(), OPERATION_EQUALS_GREATER);
                } else {
                  mstack[mstack.size() - 2]->add_nocopy(mstack.back(),
                                                        OPERATION_GREATER);
                }
              ...
              case '\x1c': {
                if (po.angle_unit != ANGLE_UNIT_NONE &&
                    ...
                    mstack.back()->contains(getRadUnit(), false, true, true) <=
                        0 &&
                    mstack.back()->contains(getGraUnit(), false, true, true) <=
                        0 &&
                    mstack.back()->contains(getDegUnit(), false, true, true) <=
                        0) {
                  switch (po.angle_unit) {
                  case ANGLE_UNIT_DEGREES: {
                    mstack.back()->multiply(getDegUnit());
                    break;
                  }
                  case ANGLE_UNIT_GRADIANS: {
                    mstack.back()->multiply(getGraUnit());
                    break;
                  }
                  default: {
                  }
                ...
                mstack[mstack.size() - 2]->transform_nocopy(
                    STRUCT_MULTIPLICATION, mstack.back());
                mstack.pop_back();
                break;
              }
              case '\a': {
                mstack[mstack.size() - 2]->transform_nocopy(STRUCT_BITWISE_XOR,
                                                            mstack.back());
                mstack.pop_back();
                break;
              }
              ...
              default: {
                error(true, _("RPN syntax error. Operator '%c' not supported."),
                      last_operator, NULL);
                mstack.pop_back();
                break;
              }
              }
              if (last_operator == NOT_CH || last_operator == BITWISE_NOT_CH)
                break;
            }
          }
        } else if (mstack.size() == 1) {
      ...
      if (!str2.empty()) {
        ...
        if ((str[i] == GREATER_CH || str[i] == LESS_CH) && po2.base < 10 &&
            po2.base >= 2 && i + 1 < str.length() && str[i + 1] == str[i] &&
            str2.find_first_not_of(NUMBERS SPACE PLUS MINUS) == string::npos) {
      ...
      if (str[i] != SPACE_CH) {
        if (mstack.size() < 1) {
          ...
          if (str[i] == NOT_CH) {
            ...
            if (po.angle_unit != ANGLE_UNIT_NONE &&
                po.angle_unit != ANGLE_UNIT_RADIANS &&
                mstack.back()->contains(getRadUnit(), false, true, true) <= 0 &&
                mstack.back()->contains(getGraUnit(), false, true, true) <= 0 &&
                mstack.back()->contains(getDegUnit(), false, true, true) <= 0) {
              switch (po.angle_unit) {
              case ANGLE_UNIT_DEGREES: {
                mstack.back()->multiply(getDegUnit());
                break;
            ...
            if (po.preserve_format)
              mstack.back()->swapChildren(1, 2);
          } else {
            error(false,
                  _("RPN syntax error. Operator ignored as there where only "
                    "one stack value."),
                  NULL);
          }
        } else {
          ...
          case AND_CH: {
            if (i + 1 < str.length() && str[i + 1] == AND_CH) {
              mstack[mstack.size() - 2]->transform_nocopy(STRUCT_LOGICAL_AND,
                                                          mstack.back());
            } else {
              mstack[mstack.size() - 2]->transform_nocopy(STRUCT_BITWISE_AND,
                                                          mstack.back());
          ...
          case OR_CH: {
            if (i + 1 < str.length() && str[i + 1] == OR_CH) {
              mstack[mstack.size() - 2]->transform_nocopy(STRUCT_LOGICAL_OR,
                                                          mstack.back());
            } else {
              mstack[mstack.size() - 2]->transform_nocopy(STRUCT_BITWISE_OR,
                                                          mstack.back());
          ...
          case '\x1c': {
            ...
            mstack.back()->transform(priv->f_cis);
            mstack[mstack.size() - 2]->transform_nocopy(STRUCT_MULTIPLICATION,
                                                        mstack.back());
            mstack.pop_back();
            break;
          }
          ...
          last_operator = str[i];
          if (i + 1 < str.length())
            last_operator2 = str[i + 1];
          else
            last_operator2 = 0;
          if ((last_operator2 == EQUALS_CH &&
               (last_operator == GREATER_CH || last_operator == LESS_CH ||
                last_operator == EQUALS_CH)) ||
              (last_operator2 == NOT_CH && last_operator == EQUALS_CH) ||
              (last_operator == last_operator2 &&
               (last_operator == GREATER_CH || last_operator == LESS_CH ||
                last_operator == '%' || last_operator == AND_CH ||
                last_operator == OR_CH))) {
            i++;
          }
        }
  ...
  if (po.base != BASE_ROMAN_NUMERALS &&
      (i = str.find(LOGICAL_OR, 1)) != string::npos && i + 2 != str.length()) {
    bool b = false, append = false;
    while (i != string::npos && i + 2 != str.length()) {
      str2 = str.substr(0, i);
      str = str.substr(i + 2, str.length() - (i + 2));
  ...
  /*if((i = str.find(LOGICAL_XOR, 1)) != string::npos && i + strlen(LOGICAL_XOR)
  != str.length()) { str2 = str.substr(0, i); str = str.substr(i +
  strlen(LOGICAL_XOR), str.length() - (i + strlen(LOGICAL_XOR))); parseAdd(str2,
  mstruct, po); parseAdd(str, mstruct, po, OPERATION_LOGICAL_XOR); return true;
  }*/
  if (po.base != BASE_ROMAN_NUMERALS &&
      (i = str.find(BITWISE_OR, 1)) != string::npos && i + 1 != str.length()) {
    bool b = false, append = false;
  ...
  if ((i = str.find_first_of(LESS GREATER EQUALS NOT, 0)) != string::npos) {
    while (
        i != string::npos &&
        ((str[i] == LOGICAL_NOT_CH &&
          (i + 1 >= str.length() || str[i + 1] != EQUALS_CH)) ||
         (str[i] == LESS_CH && i + 1 < str.length() && str[i + 1] == LESS_CH) ||
         (str[i] == GREATER_CH && i + 1 < str.length() &&
          str[i + 1] == GREATER_CH))) {
  ...
  if (i != string::npos) {
    bool b = false;
    bool c = false;
    while (i != string::npos && str[i] == NOT_CH && str.length() > i + 1 &&
           str[i + 1] == NOT_CH) {
      i++;
      if (i + 1 == str.length()) {
        c = true;
    ...
    while (!c) {
      ...
      if (b) {
        ...
        case GREATER_CH *EQUALS_CH: {
          s = OPERATION_EQUALS_GREATER;
          break;
        }
        case LESS_CH *EQUALS_CH: {
          s = OPERATION_EQUALS_LESS;
          break;
        }
        case GREATER_CH *LESS_CH: {
          s = OPERATION_NOT_EQUALS;
          break;
        }
        }
  ...
  if (i != string::npos && i + 2 != str.length()) {
    ...
    if (po.base < 10 && po.base >= 2 &&
        str.find_first_not_of(NUMBERS SPACE PLUS MINUS) == string::npos) {
  ...
  if ((i = str.find_first_of(PLUS MINUS, 1)) != string::npos &&
    ...
    bool min = false;
    while (i != string::npos && i + 1 != str.length()) {
      if (is_not_in(MULTIPLICATION_2 OPERATORS INTERNAL_OPERATORS EXPS,
                    str[i - 1])) {
        str2 = str.substr(0, i);
        if (!c && b) {
          bool b_add;
          if (min) {
            b_add = parseAdd(str2, mstruct, po, OPERATION_SUBTRACT, append) &&
                    mstruct->isAddition();
          } else {
            b_add = parseAdd(str2, mstruct, po, OPERATION_ADD, append) &&
                    mstruct->isAddition();
          }
          append = true;
          if (b_add) {
            int i_neg = 0;
            MathStructure *mstruct_a =
                get_out_of_negate(mstruct->last(), &i_neg);
            MathStructure *mstruct_b = mstruct_a;
            if (mstruct_a->isMultiplication() && mstruct_a->size() >= 2)
              mstruct_b = &mstruct_a->last();
            if (mstruct_b->isVariable() &&
                (mstruct_b->variable() == v_percent ||
                 mstruct_b->variable() == v_permille ||
                 mstruct_b->variable() == v_permyriad)) {
              ...
              if (mstruct->last()[0].isNumber()) {
                if (b_neg)
                  mstruct->last()[0].number().negate();
                if (v == v_percent)
                  mstruct->last()[0].number().add(100);
                else if (v == v_permille)
                  mstruct->last()[0].number().add(1000);
                else
                  mstruct->last()[0].number().add(10000);
              } else {
                if (b_neg && po.preserve_format)
                  mstruct->last()[0].transform(STRUCT_NEGATE);
                else if (b_neg)
                  mstruct->last()[0].negate();
                if (v == v_percent)
                  mstruct->last()[0] += Number(100, 1);
                else if (v == v_permille)
                  mstruct->last()[0] += Number(1000, 1);
                else
                  mstruct->last()[0] += Number(10000, 1);
    ...
    if (b) {
      if (c) {
        ...
        bool b_add;
        if (min) {
          b_add = parseAdd(str, mstruct, po, OPERATION_SUBTRACT, append) &&
                  mstruct->isAddition();
        } else {
          b_add = parseAdd(str, mstruct, po, OPERATION_ADD, append) &&
                  mstruct->isAddition();
        }
        if (b_add) {
          int i_neg = 0;
  ...
  if (!po.rpn && po.parsing_mode == PARSING_MODE_ADAPTIVE &&
      (i = str.find(DIVISION_CH, 1)) != string::npos && i + 1 != str.length()) {
    while (i != string::npos && i + 1 != str.length()) {
      bool b = false;
      if (i > 2 && i < str.length() - 3 && str[i + 1] == ID_WRAP_LEFT_CH) {
        ...
        while (b) {
          ...
          if (i2 > 2 && str[i2 - 1] == ID_WRAP_RIGHT_CH) {
            b = true;
          } else if (i2 > 4 && str[i2 - 3] == ID_WRAP_RIGHT_CH &&
                     str[i2 - 2] == POWER_CH && is_in(NUMBERS, str[i2 - 1])) {
            b = true;
            i4 -= 2;
          }
          if (!b) {
            if ((i2 > 1 &&
                 is_not_in(OPERATORS INTERNAL_OPERATORS MULTIPLICATION_2,
                           str[i2 - 1])) ||
                (i2 > 2 && str[i2 - 1] == MULTIPLICATION_2_CH &&
                 is_not_in(OPERATORS INTERNAL_OPERATORS, str[i2 - 2])))
              had_nonunit = true;
            break;
          }
        ...
        while (b) {
          ...
          if (i3 < str.length() - 3 && str[i3 + 1] == ID_WRAP_LEFT_CH) {
            b = true;
          } else if (i3 < str.length() - 5 && str[i3 + 3] == ID_WRAP_LEFT_CH &&
                     str[i3 + 1] == POWER_CH && is_in(NUMBERS, str[i3 + 2])) {
            b = true;
            i3 += 2;
          }
        }
        b = had_unit;
        if (b) {
          if (i3 < str.length() - 2 && str[i3 + 1] == POWER_CH &&
              is_in(NUMBERS, str[i3 + 2])) {
            i3 += 2;
            while (i3 < str.length() - 1 && is_in(NUMBERS, str[i3 + 1]))
              i3++;
          }
          if (i3 == str.length() - 1 ||
              (str[i3 + 1] != POWER_CH && str[i3 + 1] != DIVISION_CH)) {
            MathStructure *mstruct2 = new MathStructure();
      ...
      if (!b) {
        i2 = str.find_last_not_of(NUMBERS INTERNAL_NUMBER_CHARS PLUS MINUS EXPS,
                                  i - 1);
        if (i2 == string::npos ||
            (i2 != i - 1 && str[i2] == MULTIPLICATION_2_CH))
          b = true;
        i2 = str.rfind(MULTIPLICATION_2_CH, i - 1);
        if (i2 == string::npos)
          b = true;
        if (b) {
          i3 = str.find_first_of(MULTIPLICATION_2 "%" MULTIPLICATION DIVISION,
                                 i + 1);
          if (i3 == string::npos || i3 == i + 1 ||
              str[i3] != MULTIPLICATION_2_CH)
            b = false;
          if (i3 < str.length() + 1 &&
              (str[i3 + 1] == '%' || str[i3 + 1] == DIVISION_CH ||
               str[i3 + 1] == MULTIPLICATION_CH || str[i3 + 1] == POWER_CH))
            b = false;
        }
        if (b) {
          if (i3 != string::npos)
            str[i3] = MULTIPLICATION_CH;
          if (i2 != string::npos)
            str[i2] = MULTIPLICATION_CH;
        } else {
  ...
  if (po.parsing_mode == PARSING_MODE_ADAPTIVE && !po.rpn)
    remove_blanks(str);
  if (po.parsing_mode == PARSING_MODE_CONVENTIONAL) {
    if ((i = str.find(ID_WRAP_RIGHT_CH, 1)) != string::npos &&
        i + 1 != str.length()) {
      while (i != string::npos && i + 1 != str.length()) {
  ...
  if ((i = str.find_first_of(MULTIPLICATION DIVISION "%", 0)) != string::npos &&
      i + 1 != str.length()) {
    bool b = false, append = false;
    int type = 0;
    while (i != string::npos && i + 1 != str.length()) {
      if (i < 1) {
        if (i < 1 && str.find_first_not_of(
                         MULTIPLICATION_2 OPERATORS INTERNAL_OPERATORS EXPS) ==
                         string::npos) {
          gsub("\a",
               str.find_first_of(OPERATORS "%") != string::npos ? " xor "
                                                                : "xor",
               str);
          error(false, _("Misplaced operator(s) \"%s\" ignored"), str.c_str(),
                NULL);
          return b;
        }
        i = 1;
        ...
        gsub("\a",
             str.find_first_of(OPERATORS "%") != string::npos ? " xor " : "xor",
             errstr);
        error(false, _("Misplaced operator(s) \"%s\" ignored"), errstr.c_str(),
              NULL);
        ...
        if (is_in(MULTIPLICATION DIVISION "%", str[i + 1])) {
          i2 = 1;
          while (i2 + i + 1 != str.length() &&
                 is_in(MULTIPLICATION DIVISION "%", str[i2 + i + 1])) {
            i2++;
          }
          ...
          gsub("\a",
               str.find_first_of(OPERATORS "%") != string::npos ? " xor "
                                                                : "xor",
               errstr);
          error(false, _("Misplaced operator(s) \"%s\" ignored"),
                errstr.c_str(), NULL);
          i += i2;
        }
        str = str.substr(i + 1, str.length() - (i + 1));
  ...
  if ((i = str.find('\x1c', 0)) != string::npos && i + 1 != str.length()) {
    ...
    if (po.angle_unit != ANGLE_UNIT_NONE &&
        po.angle_unit != ANGLE_UNIT_RADIANS &&
        mstruct->last().contains(getRadUnit(), false, true, true) <= 0 &&
        mstruct->last().contains(getGraUnit(), false, true, true) <= 0 &&
        mstruct->last().contains(getDegUnit(), false, true, true) <= 0) {
      switch (po.angle_unit) {
      case ANGLE_UNIT_DEGREES: {
        mstruct->last().multiply(getDegUnit());
        break;
      }
      case ANGLE_UNIT_GRADIANS: {
        mstruct->last().multiply(getGraUnit());
        break;
      }
      default: {
      }
  ...
  if (str.empty())
    return false;
  if (str.find_first_not_of(OPERATORS INTERNAL_OPERATORS SPACE) ==
          string::npos &&
      (po.base != BASE_ROMAN_NUMERALS ||
  ...
  while (i < str.length()) {
    if (str[i] == MINUS_CH) {
      ...
      break;
    } else if (is_in(OPERATORS INTERNAL_OPERATORS, str[i]) &&
               (po.base != BASE_ROMAN_NUMERALS ||
                (str[i] != '(' && str[i] != ')' && str[i] != '|'))) {
      if (str[i] == '\a')
        error(false, _("Misplaced operator(s) \"%s\" ignored"), "xor", NULL);
  ...
  if ((i = str.find(ID_WRAP_RIGHT_CH, 1)) != string::npos &&
    ...
    if (b) {
      parseAdd(str, mstruct, po, OPERATION_MULTIPLY, append);
      if (po.parsing_mode == PARSING_MODE_ADAPTIVE &&
          mstruct->isMultiplication() && mstruct->size() >= 2 &&
          !(*mstruct)[0].inParentheses()) {
        Unit *u1 = NULL;
        Prefix *p1 = NULL;
        bool b_plus = false;
        if ((*mstruct)[0].isMultiplication() && (*mstruct)[0].size() == 2 &&
            (*mstruct)[0][0].isNumber() && (*mstruct)[0][1].isUnit()) {
          u1 = (*mstruct)[0][1].unit();
          p1 = (*mstruct)[0][1].prefix();
        }
        if (u1 && u1->subtype() == SUBTYPE_BASE_UNIT &&
            (u1->referenceName() == "m" ||
             (!p1 && u1->referenceName() == "L")) &&
            (!p1 || (p1->type() == PREFIX_DECIMAL &&
                     ((DecimalPrefix *)p1)->exponent() <= 3 &&
                     ((DecimalPrefix *)p1)->exponent() > -3))) {
          b_plus = true;
          for (size_t i2 = 1; i2 < mstruct->size(); i2++) {
            if (!(*mstruct)[i2].inParentheses() &&
                (*mstruct)[i2].isMultiplication() &&
                (*mstruct)[i2].size() == 2 && (*mstruct)[i2][0].isNumber() &&
                (*mstruct)[i2][1].isUnit() && (*mstruct)[i2][1].unit() == u1) {
              Prefix *p2 = (*mstruct)[i2][1].prefix();
              if (p1 && p2)
                b_plus = p1->type() == PREFIX_DECIMAL &&
                         p2->type() == PREFIX_DECIMAL &&
                         ((DecimalPrefix *)p1)->exponent() >
                             ((DecimalPrefix *)p2)->exponent() &&
                         ((DecimalPrefix *)p2)->exponent() >= -3;
              else if (p2)
                b_plus = p2->type() == PREFIX_DECIMAL &&
                         ((DecimalPrefix *)p2)->exponent() < 0 &&
                         ((DecimalPrefix *)p2)->exponent() >= -3;
              else if (p1)
                b_plus = p1->type() == PREFIX_DECIMAL &&
                         ((DecimalPrefix *)p1)->exponent() > 1;
              else
                b_plus = false;
              if (!b_plus)
                break;
        ...
        } else if (u1 && !p1 && u1->subtype() == SUBTYPE_ALIAS_UNIT &&
                   ((AliasUnit *)u1)->mixWithBase()) {
          b_plus = true;
          for (size_t i2 = 1; i2 < mstruct->size(); i2++) {
            if (!(*mstruct)[i2].inParentheses() &&
                (*mstruct)[i2].isMultiplication() &&
                (*mstruct)[i2].size() == 2 && (*mstruct)[i2][0].isNumber() &&
                (*mstruct)[i2][1].isUnit() &&
                u1->isChildOf((*mstruct)[i2][1].unit()) &&
                !(*mstruct)[i2][1].prefix() &&
                (i2 == mstruct->size() - 1 ||
                 ((*mstruct)[i2][1].unit()->subtype() == SUBTYPE_ALIAS_UNIT &&
                  ((AliasUnit *)(*mstruct)[i2][1].unit())->mixWithBase()))) {
              while (((AliasUnit *)u1)->firstBaseUnit() !=
                     (*mstruct)[i2][1].unit()) {
                u1 = ((AliasUnit *)u1)->firstBaseUnit();
                if (u1->subtype() != SUBTYPE_ALIAS_UNIT ||
                    !((AliasUnit *)u1)->mixWithBase()) {
                  b_plus = false;
                  break;
                }
  ...
  if ((i = str.find(ID_WRAP_LEFT_CH, 1)) != string::npos) {
    ...
    while (i != string::npos) {
      if (str[i - 1] != POWER_CH &&
          (i < 2 || str[i - 1] != MINUS_CH || str[i - 2] != POWER_CH) &&
          str[i - 1] != '\b') {
        str2 = str.substr(0, i);
        str = str.substr(i, str.length() - i);
        if (b) {
  ...
  if ((i = str.find(POWER_CH, 1)) != string::npos && i + 1 != str.length()) {
    ...
    mstruct->addChild_nocopy(mstruct2);
    mstruct->addChild(m_zero);
  } else if (BASE_2_10 && (i = str.find_first_of(EXPS, 1)) != string::npos &&
             i + 1 != str.length() && str.find("\b") == string::npos) {
    str2 = str.substr(0, i);
    str = str.substr(i + 1, str.length() - (i + 1));
    parseAdd(str2, mstruct, po);
    parseAdd(str, mstruct, po, OPERATION_EXP10);
  } else if ((i = str.find(ID_WRAP_LEFT_CH, 1)) != string::npos &&
             i + 1 != str.length() && str.find(ID_WRAP_RIGHT_CH, i + 1) &&
             str.find_first_not_of(PLUS MINUS, 0) != i) {
    str2 = str.substr(0, i);
    str = str.substr(i, str.length() - i);
    parseAdd(str2, mstruct, po);
    parseAdd(str, mstruct, po, OPERATION_MULTIPLY);
  } else if (str.length() > 0 && str[0] == ID_WRAP_LEFT_CH &&
             (i = str.find(ID_WRAP_RIGHT_CH, 1)) != string::npos &&
             i + 1 != str.length()) {
    str2 = str.substr(0, i + 1);
    str = str.substr(i + 1, str.length() - (i + 1));
    parseAdd(str2, mstruct, po);
...
int Calculator::loadDefinitions(const char *file_name, bool is_user_defs,
  ...
  bool unc_rel;
  bool best_title, next_best_title, best_category_title,
...
int Calculator::savePrefixes(const char *file_name, bool save_global) {
  ...
  for (size_t i = 0; i < prefixes.size(); i++) {
    ...
    if (!prefixes[i]->longName(false).empty())
      xmlNewTextChild(newnode, NULL, (xmlChar *)"name",
                      (xmlChar *)prefixes[i]->longName(false).c_str());
    if (!prefixes[i]->shortName(false).empty())
      xmlNewTextChild(newnode, NULL, (xmlChar *)"abbreviation",
                      (xmlChar *)prefixes[i]->shortName(false).c_str());
    if (!prefixes[i]->unicodeName(false).empty())
      xmlNewTextChild(newnode, NULL, (xmlChar *)"unicode",
                      (xmlChar *)prefixes[i]->unicodeName(false).c_str());
...
int Calculator::saveVariables(const char *file_name, bool save_global) {
  ...
  for (size_t i = 0; i < variables.size(); i++) {
    if ((save_global || variables[i]->isLocal() ||
        ...
        variables[i]->category() != "Temporary") {
      item = &top;
    ...
    if (save_global || u->isLocal() || u->hasChanged()) {
      ...
      if (!save_global && !u->isLocal() && u->hasChanged()) {
        ...
        if (!u->isSIUnit() || !u->useWithPrefixesByDefault()) {
          xmlNewTextChild(newnode, NULL, (xmlChar *)"use_with_prefixes",
...
string Calculator::getExchangeRatesFileName(int index) {
  ...
  case 3: {
    return buildPath(getLocalDataDir(), "rates.html");
  }
  default: {
  }
  }
...
time_t Calculator::getExchangeRatesTime(int index) {
  ...
  if (index < 1) {
    if (exchange_rates_time[1] < exchange_rates_time[0]) {
      if (exchange_rates_time[2] < exchange_rates_time[1])
        return exchange_rates_time[2];
      return exchange_rates_time[1];
    }
    if (exchange_rates_time[2] < exchange_rates_time[0])
      return exchange_rates_time[2];
...
string Calculator::getExchangeRatesUrl(int index) {
  ...
  case 3: {
    return "https://www.mycurrency.net/=US";
  }
  default: {
  }
  }
...
bool Calculator::fetchExchangeRates(int timeout, int n) {
  ...
  if (res != CURLE_OK) {
    if (strlen(error_buffer))
      error(true, _("Failed to download exchange rates from %s: %s."), "ECB",
            error_buffer, NULL);
    else
      error(true, _("Failed to download exchange rates from %s: %s."), "ECB",
  ...
  if (n >= 2) {
    ...
    if (res != CURLE_OK) {
      error(true, _("Failed to download exchange rates from %s: %s."),
            "coinbase.com", error_buffer, NULL);
      FETCH_FAIL_CLEANUP;
      return false;
    }
    if (sbuffer.empty()) {
      error(true, _("Failed to download exchange rates from %s: %s."),
            "coinbase.com", "Document empty", NULL);
      FETCH_FAIL_CLEANUP;
      return false;
    }
    ...
    if (!file3.is_open()) {
      error(true, _("Failed to download exchange rates from %s: %s."),
            "coinbase.com", strerror(errno), NULL);
      FETCH_FAIL_CLEANUP
      return false;
    }
    file3 << sbuffer;
  ...
  if (n >= 3) {
    ...
    if (res != CURLE_OK) {
      error(true, _("Failed to download exchange rates from %s: %s."),
            "mycurrency.net", error_buffer, NULL);
      FETCH_FAIL_CLEANUP;
      return false;
    }
    if (sbuffer.empty() ||
        sbuffer.find("Internal Server Error") != string::npos) {
      error(true, _("Failed to download exchange rates from %s: %s."),
            "mycurrency.net", "Document empty", NULL);
      FETCH_FAIL_CLEANUP;
      return false;
    }
    ...
    if (!file2.is_open()) {
      error(true, _("Failed to download exchange rates from %s: %s."),
            "mycurrency.net", strerror(errno), NULL);
      FETCH_FAIL_CLEANUP
      return false;
    }
    file2 << sbuffer;
...
bool Calculator::checkExchangeRatesDate(unsigned int n_days, bool force_check,
  ...
  if (n > 1 && exchange_rates_time[1] < extime)
    extime = exchange_rates_time[1];
  if (n > 2 && exchange_rates_time[2] < extime)
    extime = exchange_rates_time[2];
  time_t cextime = exchange_rates_check_time[0];
  if (n > 1 && exchange_rates_check_time[1] < cextime)
    cextime = exchange_rates_check_time[1];
  if (n > 2 && exchange_rates_check_time[2] < cextime)
    cextime = exchange_rates_check_time[2];
  ...
  if (send_warning)
    error(
        ...
        i2s((int)floor(difftime(time(NULL), extime) / 86400)).c_str(), NULL);
  return false;
}
...
extern bool fix_intervals(MathStructure &mstruct, const EvaluationOptions &eo,
...
Calculator::expressionToPlotVector(string expression, const MathStructure &min,
  ...
  MathStructure y_vector(
      mparse.generateVector(x_mstruct, min, max, steps, x_vector, eo));
  endTemporaryStopMessages();
...
MathStructure Calculator::expressionToPlotVector(
  ...
  beginTemporaryStopMessages();
  MathStructure y_vector(
      mparse.generateVector(x_mstruct, min, max, step, x_vector, eo));
  endTemporaryStopMessages();
  endTemporaryStopIntervalArithmetic();
  if (msecs > 0) {
    if (aborted())
    ...
    if (!y_vectors[serie].isUndefined()) {
      ...
      for (size_t i = 1; i <= y_vectors[serie].countChildren(); i++) {
        ...
        if (!y_vectors[serie].getChild(i)->isNumber()) {
          invalid_nr = true;
          non_numerical++;
          // if(non_numerical == 1) str =
          // y_vectors[serie].getChild(i)->print(po);
        } else if (!y_vectors[serie].getChild(i)->number().isReal()) {
          b_imagzero_y = testComplexZero(
              &y_vectors[serie].getChild(i)->number(),
              y_vectors[serie].getChild(i)->number().internalImaginary());
          if (!b_imagzero_y) {
            invalid_nr = true;
            non_real++;
            // if(non_numerical + non_real == 1) str =
            // y_vectors[serie].getChild(i)->print(po);
          }
        }
        if (serie < x_vectors.size() && !x_vectors[serie].isUndefined() &&
          ...
          if (!x_vectors[serie].getChild(i)->isNumber()) {
            ...
            non_numerical++;
            // if(non_numerical == 1) str =
            // x_vectors[serie].getChild(i)->print(po);
        ...
        if (!invalid_nr) {
          if (check_continuous && !prev_failed) {
            if (i == 1 || ct2 == COMPARISON_RESULT_UNKNOWN)
              ...
              ct = y_vectors[serie].getChild(i - 1)->number().compare(
                  y_vectors[serie].getChild(i)->number());
          ...
          if (b_imagzero_x)
            plot_data +=
                y_vectors[serie].getChild(i)->number().realPart().print(po);
          else
      ...
      /*if(non_numerical > 0 || non_real > 0) {
          ...
          if(non_numerical > 0) {
                  error(true, _("Series %s contains non-numerical data (\"%s\"
      first of %s) which can not be properly plotted."), stitle.c_str(),
      str.c_str(), i2s(non_numerical).c_str(), NULL); } else { error(true,
      _("Series %s contains non-real data (\"%s\" first of %s) which can not be
      properly plotted."), stitle.c_str(), str.c_str(), i2s(non_real).c_str(),
      NULL);
          }
      }*/
